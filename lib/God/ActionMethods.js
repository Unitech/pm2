...
module.exports = function(God) {
  ...
  God.startProcessId = function(id, cb) {
    ...
    if (proc.process && proc.process.pid)
      return cb('Process with pid ' + proc.process.pid + ' already exists');
  ...
  God.stopProcessId = function(id, cb) {
    ...
    if (God.clusters_db[id].pm2_env.status === cst.LAUNCHING_STATUS
      ...
      console.log('DEBUG MARK HERE'); // DEBUG
      return;
    }
  ...
  God.restartProcessId = function(opts, cb) {
    ...
    if (God.pm2_being_killed) {
      return cb('[RestartProcessId] PM2 is being killed, stopping restart procedure...');
    }
    if (proc.pm2_env.status === cst.ONLINE_STATUS) {
