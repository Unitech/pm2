...
module.exports = function(God) {
  ...
  God.getMonitorData = function getMonitorData(env, cb) {
    var processes = God.getFormatedProcesses();
	  var empty_data = { memory : 0, cpu : 0 }

    async.each(processes, function computeMonitor(pro, callback) {
    ...
    }, function retMonitor( err ) {
      if (err) 
        return cb(God.logAndGenerateError(err), null);
  ...
  God.reloadProcessName = function(name, cb) {
    ...
    async.eachLimit(processes, cst.CONCURRENT_ACTIONS, function(proc, next) {
      ...
      if (proc.pm2_env.status === cst.ONLINE_STATUS)
        return God.reloadProcessId({id:proc.pm2_env.pm_id}, next);
