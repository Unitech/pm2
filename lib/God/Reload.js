...
function hardReload(God, id, wait_msg, cb) {
  ...
  God.executeApp(new_env, function(err, new_worker) {
    ...
    timer = setTimeout(function() {
      ...
      if (!new_env.reload_after_ready) {
        God.deleteProcessId(t_key, cb);
      } else {
        God.deleteProcessId(new_worker.pm2_env.pm_id, function (err, proc) {
          if (err) return cb(err);
...
module.exports = function(God) {
  ...
  God.reloadProcessId = function(opts, cb) {
    ...
    if (proc.pm2_env.status == cst.ONLINE_STATUS &&
      ...
      var wait_msg = (proc.pm2_env.wait_ready || proc.pm2_env.reload_after_ready) ? 'ready' : 'listening';
