...
module.exports = function(God) {
  ...
  */
  God.watch = function(pm2_env) {
    var watch = require('watch'), firstlaunch = true;
    watch.watchTree(
      p.dirname(pm2_env.pm_exec_path),
      { 'ignoreDotFiles' : true },
      function (f, curr, prev) {
        if(!firstlaunch && God.findProcessById(pm2_env.pm_id).pm2_env.watch === true) {
          console.log('File changed. Restarting id:%d (--watch has been enabled).', pm2_env.pm_id);
          God.restartProcessId(pm2_env.pm_id, function(err, list) {
            console.log('Process successfully restarted');
          });
        } else
          firstlaunch = false;
    });
