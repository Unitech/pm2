...
module.exports = function(God) {
  ...
  God.processIsDead = function(pid, pm2_env, cb, sigkill) {
    ...
    // _tree_pids only has to outlive this kill sequence. Left on pm2_env it
    // would follow the process into its next incarnation: exported to the
    // cluster worker's process.env by ProcessContainer.js, and reused as the
    // pid list of a later kill that skips treekill (shutdown_with_message).
    function done(err, res) {
      delete pm2_env._tree_pids;
      return cb(err, res);
    }
    ...
    timeout = setTimeout(function() {
      ...
      if (sigkill) {
        ...
        return done({type : 'timeout', msg : 'timeout'});
      }
      else {
        var alive = survivors();
