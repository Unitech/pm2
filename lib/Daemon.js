...
Daemon.prototype.start = function() {
  ...
  d.once('error', function(err) {
    ...
    var fork_new_pm2 = require('child_process').spawn('node', [process.env['_'], 'update'], {
      detached: true,
      stdio: 'inherit'
    });
...
Daemon.prototype.innerStart = function(cb) {
  ...
  if (!cb) cb = function() {
    ...
    fmt.field('Instance owner', process.versions.node);
  ...
  this.pub_socket.once('bind', function() {
    console.log('BUS system binded to socket %s', that.pub_socket_file);
  ...
  this.rpc_socket.once('bind', function() {
    console.log('RPC interface binded to socket %s', that.rpc_socket_file);
