...
Common.prepareAppConf = function(opts, app) {
  ...
  function filterEnv(envObj){
    ...
    let allowedKeys = Object.keys(envObj).filter(
      item => opts.filterEnv.find( env => env.charAt(0) === "-" ? !item.includes(env) : item.includes(env))
    );
    allowedKeys.forEach( key => new_env[key] = envObj[key]);
    return new_env
  }

  app.env = [{}, opts.filterEnv && opts.filterEnv.length ? filterEnv(process.env) : env, app.env || {}].reduce(function(e1, e2){
