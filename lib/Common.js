...
Common.prepareAppConf = function(opts, app) {
  ...
  function filterEnv(envObj){
    console.log('$ pm2 logssssss1 ' + envObj);
    console.log('$ pm2 logsss-----sss1 ' + app.filter_env);
    let new_env = {};
    let allowedKeys = Object.keys(envObj).filter(
      item => app.filter_env.find( env => !item.includes(env))
    );
  ...
  console.log('$ pm2 logssssss ' + process.env);
  console.log('$ pm2 logsss-----sss ' + app.filter_env);
