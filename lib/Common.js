...
var fclone    = require('../modules/fclone');
...
var pm2Keys   = require('./tools/pm2-config-keys.js');
...
Common.prepareAppConf = function(opts, app) {
  ...
  // The ambient environment must not supply PM2 options: a stray `name=` or
  // `port=` variable would otherwise become app configuration. Strip them here,
  // at ingestion, while the two origins are still distinguishable -- `env:` is
  // layered on top afterwards, so what the user declares is kept as-is and
  // reaches the child (#6006 #6016 #5747 #5622 #5030 #5032 #4943 #4778 #4821 #3210)
  var shell_env = (app.filter_env && app.filter_env.length > 0) ? filterEnv(process.env) : env;

  app.env = [
    {}, pm2Keys.omitPm2ConfigKeys(shell_env), app.env || {}
...
Common.sink.determineExecMode = function(app) {
   ...
   */
  var interpreterBasename = path.basename(app.exec_interpreter || '');
  var isNodeInterpreter = interpreterBasename === 'node' || /node$/.test(interpreterBasename) ||
      interpreterBasename === 'bun' || /bun$/.test(interpreterBasename);
  ...
  // Cluster mode require()s the script inside a Node.js worker: a binary or
  // a script run by another interpreter (bash, python...) would fail with
  // a SyntaxError on its first bytes (#4775) — fall back to fork mode
  if (app.exec_mode === 'cluster_mode' && !isNodeInterpreter) {
    Common.printError(cst.PREFIX_MSG_WARNING + chalk.bold.yellow(
      `Cluster mode is only available for Node.js/Bun scripts (${app.pm_exec_path} runs with interpreter '${app.exec_interpreter}'), falling back to fork mode`));
    app.exec_mode = 'fork_mode';
  }
...
/**
 * Copy `add` into `origin`, minus everything PM2 owns.
 *
 * Used to lift an environment out of a pm2_env-shaped object: when starting a
 * script programmatically from inside a managed app, and when listing an app's
 * env back to the user. The key list is shared with the daemon -- it used to be
 * hand-maintained here and had drifted (no `port`, `time`, `trace`, `uid`/`gid`,
 * no `pm_*`/`axm_*` prefixes).
 */
Common.safeExtend = function(origin, add){
  ...
  var i = keys.length;
  while (i--) {
    if (!pm2Keys.isPm2ConfigKey(keys[i]) && add[keys[i]] != '[object Object]')
...
Common.verifyConfs = function(appConfs) {
  ...
  for (var i = 0; i < appConfs.length; i++) {
    ...
    if (app.trace === true || app.deep_monitoring === true) {
      var OtelManager = require('./OtelManager');
      if (!OtelManager.ensureInstalled()) {
        Common.printError(cst.PREFIX_MSG_ERR + 'Tracing disabled: OpenTelemetry packages not available');
        app.trace = false;
        delete app.deep_monitoring;
      }
    }
