  ...
  ['log', 'out', 'error', 'pid'].forEach(function(f){
    ...
    if ((f == 'log' && typeof af == 'boolean' && af) || (f != 'log' && !af)) {
      ...
      if (!fs.existsSync(path.dirname(af))) {
        Common.printError(cst.PREFIX_MSG_WARNING + 'Folder does not exists: ' + path.dirname(af));
  ...
  return new Promise(function(resolve) {
    if (!filename || filename == 'pipe' || filename == 'none' ||
      ...
      if (semver.satisfies(process.version, '>= 0.12.0')) {
        resolve(vm.runInThisContext(code, sandbox, {
        ...
        resolve(vm.runInNewContext(code, sandbox, filename));
      }
    }
    else if (filename.indexOf('.yml') > -1 ||
      filename.indexOf('.yaml') > -1) {
      resolve(yamljs.parse(confObj.toString()));
    }
    else if (filename.indexOf('.config.js') > -1) {
      var confPath = require.resolve(path.resolve(filename));
      delete require.cache[confPath];
      // Probably a promise
      if (typeof require(confPath).then === 'function') {
        require(confPath).then(function (conf) {
          resolve(conf)
        })
      } else {
        resolve(require(confPath));
      }
    }
  })
};
