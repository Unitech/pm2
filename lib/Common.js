...
Common.parseConfig = function(confObj, filename) {
  ...
  if (!filename || filename == 'pipe' || filename == 'none' ||
    ...
    if (semver.satisfies(process.version, '>= 0.12.0')) {
      return vm.runInThisContext(code, vmOptions);
    } else {
      // Use the Node 0.10 API
      return vm.runInNewContext(code, sandbox, vmOptions);
    }
  }
...
function checkExecMode(conf) {
  ...
  if (!isNaN(conf.instances) && conf.exec_mode === 'fork_mode') {
  ...
  if (conf.exec_mode === 'cluster_mode' &&
