  ...
  _startJson (file, opts, action, pipe, cb) {
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, raw_proc_list) {
      ...
      if (appConf.some(env => { if (env.priority) return true; })) {
        async.eachSeries(Object.keys(proc_list), iterFunctionForAsync, callbackAsync);        
      } else {
        async.eachLimit(Object.keys(proc_list), conf.CONCURRENT_ACTIONS, iterFunctionForAsync, callbackAsync);
      }
      return false;

      function iterFunctionForAsync(proc_name, next) {
    ...
    function startApps(app_name_to_start, cb) {
      ...
      if (apps_to_start.some(env => { if (env.priority) return true; })) {
        ...
        async.eachSeries(apps_to_start, iterForAll, function(err) {
          return cb ? cb(err || null, apps_started) : that.speedList();
        });
      } else {
        async.eachLimit(apps_to_start, conf.CONCURRENT_ACTIONS, iterForAll, function(err) {
          return cb ? cb(err || null, apps_started) : that.speedList();
        });
      }
      return false;
