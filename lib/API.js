...
var IMMUTABLE_MSG = chalk.bold.blue(
...
class API {
  constructor(opts) {
    ...
    this.daemon_mode =
      typeof opts.daemon_mode == "undefined" ? true : opts.daemon_mode;
        ...
        function tryToExit() {
          if (fds & 1 && fds & 2) {
            debug(
              ...
              (new Date() - that.start_timer) / 1000
            );
            process.exit(code);
          }
        }
  ...
  start(cmd, opts, cb) {
    ...
    if (util.isArray(opts.watch) && opts.watch.length === 0)
      opts.watch =
        (opts.rawArgs
          ? !!~opts.rawArgs.indexOf("--watch")
          : !!~process.argv.indexOf("--watch")) || false;
        ...
        function (id, next) {
          that.Client.executeRemote(
            ...
            function (err, res) {
              ...
              Common.printOut(
                conf.PREFIX_MSG + "Resetting meta for process id %d",
                id
              );
              return next();
            }
    ...
    if (process_name == "all") {
      ...
      that.Client.getProcessIdByName(process_name, function (err, ids) {
        ...
        if (ids.length === 0) {
          ...
          return cb
            ? cb(new Error("Unknown process name"))
  ...
  reload(process_name, opts, cb) {
    ...
    if (delay > 0 && opts.force != true) {
      ...
      return cb
        ? cb(new Error("Reload in progress"))
        : that.exitCli(conf.ERROR_EXIT);
    }
  ...
  _startScript(script, opts, cb) {
    ...
    if (opts.watchDelay) {
      if (
        typeof opts.watchDelay === "string" &&
        opts.watchDelay.indexOf("ms") !== -1
      )
     ...
     */
    if (app_conf.write) {
      var dst_path = path.join(
        process.env.PWD || process.cwd(),
        app_conf.name + "-pm2.json"
      );
      Common.printOut(
        conf.PREFIX_MSG + "Writing configuration to",
        chalk.blue(dst_path)
      );
      // pretty JSON
      try {
          ...
          if (ids.length > 0) {
            that._operate(
              ...
              function (err, list) {
                if (err) return cb(err);
                Common.printOut(
                  conf.PREFIX_MSG + "Process successfully started"
                );
                return cb(true, list);
              }
            );
    ...
    function restartExistingProcessPathOrStartNew(cb) {
      that.Client.executeRemote("getMonitorData", {}, function (err, procs) {
        ...
        procs.forEach(function (proc) {
          if (
            proc.pm2_env.pm_exec_path == full_path &&
        ...
        Common.printOut(
          conf.PREFIX_MSG +
            "Starting %s in %s (%d instance" +
            (resolved_paths.instances > 1 ? "s" : "") +
            ")",
        ...
        that.Client.executeRemote(
          ...
          function (err, data) {
            if (err) {
              Common.printError(
                conf.PREFIX_MSG_ERR + "Error while launching application",
                err.stack || err
              );
  ...
  _startJson(file, opts, action, pipe, cb) {
    ...
    if (typeof file === "object") {
      ...
      try {
        ...
        Common.printError(
          conf.PREFIX_MSG_ERR + "File " + file + " malformated"
        );
        console.error(e);
        return cb ? cb(Common.retErr(e)) : that.exitCli(conf.ERROR_EXIT);
      }
    ...
    staticConf.forEach(function (serve) {
      appConf.push({
        ...
        env: {
          ...
          PM2_SERVE_BASIC_AUTH_USERNAME: serve.basic_auth
            ? serve.basic_auth.username
            : null,
          PM2_SERVE_BASIC_AUTH_PASSWORD: serve.basic_auth
            ? serve.basic_auth.password
            : null,
        ...
        eachLimit(
          ...
          function (proc_name, next) {
            ...
            var envs = apps.map(function (app) {
              ...
              return Common.mergeEnvironmentVariables(
                app,
                opts.env,
                deployConf
              );
          ...
          function (err) {
            if (err)
              return cb
                ? cb(Common.retErr(err))
                : that.exitCli(conf.ERROR_EXIT);
            if (apps_name.length > 0 && action != "start")
              Common.printOut(
                conf.PREFIX_MSG_WARNING +
                  "Applications %s not running, starting...",
                apps_name.join(", ")
              );
            // Start missing apps
    ...
    function startApps(app_name_to_start, cb) {
      ...
      eachLimit(
        ...
        function (app, next) {
          ...
          var additional_env = Modularizer.getAdditionalConf(
            resolved_paths.name
          );
          Object.assign(resolved_paths.env, additional_env);

          resolved_paths.env = Common.mergeEnvironmentVariables(
            resolved_paths,
            opts.env,
            deployConf
          );
          ...
          if (resolved_paths.wait_ready) {
            Common.warn(
              `App ${resolved_paths.name} has option 'wait_ready' set, waiting for app to be ready...`
            );
          }
          that.Client.executeRemote(
            "prepare",
            ...
            function (err, data) {
              if (err) {
                Common.printError(
                  conf.PREFIX_MSG_ERR + "Process failed to launch %s",
              ...
              if (data.length === 0) {
                Common.printError(
                  conf.PREFIX_MSG_ERR + "Process config loading failed",
                  data
                );
                return next();
              }
        ...
        function (err) {
          var final_error =
            err || apps_errored.length > 0 ? apps_errored : null;
  ...
  actionFromJson(action, file, opts, jsonVia, cb) {
    ...
    if (typeof file == "object") {
      ...
      Common.printError(
        "Bad call to actionFromJson, jsonVia should be one of file, pipe"
      );
  ...
  _operate(action_name, process_name, envs, cb) {
    ...
    function processIds(ids, cb) {
      Common.printOut(
        conf.PREFIX_MSG + "Applying action %s on app [%s](ids: %s)",
        action_name,
        process_name,
        ids
      );
      ...
      eachLimit(
        ...
        function (id, next) {
          ...
          that.Client.executeRemote(action_name, opts, function (err, res) {
            if (err) {
              Common.printError(
                conf.PREFIX_MSG_ERR + "Process %s not found",
                id
              );
            ...
            res.forEach(function (proc) {
              Common.printOut(
                conf.PREFIX_MSG + "[%s](%d) \u2713",
                proc.pm2_env ? proc.pm2_env.name : process_name,
                id
              );

              if (
                action_name == "stopProcessId" &&
                proc.pm2_env &&
                ...
                Common.warn(
                  `App ${chalk.bold(
                    proc.pm2_env.name
                  )} stopped but CRON RESTART is still UP ${
                    proc.pm2_env.cron_restart
                  }`
                );
              }
    ...
    // operate using regex
       ...
       */
      var allow_module_restart =
        action_name == "restartProcessId" ? true : false;

      that.Client.getProcessIdByName(
        process_name,
        allow_module_restart,
        function (err, ids) {
          if (err) {
            Common.printError(err);
            return cb ? cb(Common.retErr(err)) : that.exitCli(conf.ERROR_EXIT);
          ...
          that.Client.getProcessIdsByNamespace(
            process_name,
            allow_module_restart,
            function (err, ns_process_ids) {
              if (err) {
                Common.printError(err);
                return cb
                  ? cb(Common.retErr(err))
              ...
              if (!ns_process_ids || ns_process_ids.length === 0) {
                Common.printError(
                  conf.PREFIX_MSG_ERR + "Process or Namespace %s not found",
                  process_name
                );
                return cb
                  ? cb(new Error("process or namespace not found"))
                  : that.exitCli(conf.ERROR_EXIT);
              }
               ...
               * if yes load configuration variables and merge with the current environment
               */
              var ns_additional_env =
                Modularizer.getAdditionalConf(process_name);
      ...
      if (
        ...
        that.Client.executeRemote("getMonitorData", {}, (err, proc_list) => {
          ...
          if (process_name > higher_id)
            return DockerMgmt.processCommand(
              that,
              higher_id,
              process_name,
              action_name,
              (err) => {
                if (err) {
                  Common.printError(
                    conf.PREFIX_MSG_ERR + (err.message ? err.message : err)
                  );
                  return cb
                    ? cb(Common.retErr(err))
                    : that.exitCli(conf.ERROR_EXIT);
  ...
  _handleAttributeUpdate(opts) {
    ...
    if (appConf instanceof Error) {
      Common.printError(
        "Error while transforming CamelCase args to underscore"
      );
      return appConf;
    }
    ...
    if (process.env.PM2_DEEP_MONITORING) appConf.deep_monitoring = true;
    ...
    function doList(err, list) {
      ...
      if (process.stdout.isTTY === false) {
        ...
        if (that.gl_interact_infos) {
          ...
          if (
            that.gl_interact_infos.info_node != "https://root.keymetrics.io"
          ) {
            dashboard_url = `${that.gl_interact_infos.info_node}/#/r/${that.gl_interact_infos.public_key}`;
          }

          Common.printOut(
            "%s PM2+ activated | Instance Name: %s | Dash: %s",
            chalk.green.bold("⇆"),
      ...
      if (that.Client.daemon_mode == false) {
        ...
        Common.printOut(
          "[--no-daemon] Exit on target PM2 exit pid=" +
            fs.readFileSync(conf.PM2_PID_FILE_PATH).toString()
        );
      ...
      // if (process.stdout.isTTY) if looking for start logs
        ...
        acted.length > 0 &&
        commander.attach === true
      ) {
        Common.info(
          `Log streaming apps id: ${chalk.cyan(
            acted.join(" ")
          )}, exit with Ctrl-C or will exit in 10secs`
        );
  ...
  scale(app_name, number, cb) {
    ...
    function rmProcs(procs, value, cb) {
      ...
      (function ex(procs, number) {
        ...
        that._operate(
          "deleteProcessId",
          procs[i++].pm2_env.pm_id,
          ex.bind(this, procs, number)
        );
      })(procs, number);
    }
    ...
    this.Client.getProcessByName(app_name, function (err, procs) {
      ...
      if (!procs || procs.length === 0) {
        Common.printError(
          conf.PREFIX_MSG_ERR + "Application %s not found",
          app_name
        );
        return cb
      ...
      if (typeof number === "string" && number.indexOf("+") >= 0) {
        ...
        if (number < 0) return rmProcs(procs, number, end);
          ...
          return cb
            ? cb(new Error("Same process number"))
            : that.exitCli(conf.ERROR_EXIT);
        }
      }
    });
  ...
  describe(pm2_id, cb) {
    ...
    that.Client.executeRemote("getMonitorData", {}, function (err, list) {
      ...
      list.forEach(function (proc) {
        if (
          (!isNaN(pm2_id) && proc.pm_id == pm2_id) ||
