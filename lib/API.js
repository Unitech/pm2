...
////////////////////////////
  ...
  killDaemon (cb) {
    ...
    Common.printOut(conf.PREFIX_MSG + '[-] Stopping Modules');
    that.killAllModules(function() {
      Common.printOut(conf.PREFIX_MSG + '[v] Modules Stopped');

      Common.printOut(conf.PREFIX_MSG + '[-] Stopping all Applications');
      that._operate('deleteProcessId', 'all', function(err, list) {
        ...
        Common.printOut(conf.PREFIX_MSG + '[-] Stopping Agent');
        that.killAgent(function(err, data) {
          //if (err) console.error(err)
          Common.printOut(conf.PREFIX_MSG + '[v] Agent Stopped');

          Common.printOut(conf.PREFIX_MSG + '[-] Stopping PM2 Daemon');
          that.Client.killDaemon(function(err, res) {
            if (err) Common.printError(err);
            Common.printOut(conf.PREFIX_MSG + '[-] PM2 Daemon Stopped');
  ...
  _startJson (file, opts, action, pipe, cb) {
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, raw_proc_list) {
      ...
      if (appConf.some(env => { if (env.priority) return true; })) {
        var sortedKeys = Config.sortNamesByPriority(appConf, Object.keys(proc_list));
        eachSeries(sortedKeys, iterFunctionForAsync, callbackAsync);        
      } else {
        eachLimit(Object.keys(proc_list), conf.CONCURRENT_ACTIONS, iterFunctionForAsync, callbackAsync);
      }
      return false;

      function iterFunctionForAsync(names, next) {

        if (!Array.isArray(names)) {
          names = [names];
        }

        async.eachLimit(names, conf.CONCURRENT_ACTIONS, forAsyncRestart, function() { next(); });

        function forAsyncRestart(proc_name, next1) {
    ...
    function startApps(app_name_to_start, cb) {
      ...
      if (apps_to_start.some(env => { if (env.priority) return true; })) {
        apps_to_start = Config.checkPriority(apps_to_start, 'start');
        eachSeries(apps_to_start, iterForAll, asyncCB);
      } else {
        eachLimit(apps_to_start, conf.CONCURRENT_ACTIONS, iterForAll, asyncCB);
      }
      return false;
      ...
      // This function for execution process with formating process

      function iterForAll(appArr, next) {
        
        if (!Array.isArray(appArr)) {
          appArr = [appArr];
        }

        async.eachLimit(appArr, conf.CONCURRENT_ACTIONS, forAsync, function() { next(); });

        function forAsync(app, next1) {
          if (opts.cwd)
            app.cwd = opts.cwd;
          if (opts.force_name)
  ...
  actionFromJson (action, file, opts, jsonVia, cb) {
    ...
    if (appConf.some(env => { if (env.stop_priority) return true; }) || appConf.some(env => { if (env.priority) return true; })) {
      appConf = Config.checkPriority(appConf, 'stop');
      eachSeries(appConf, iterForStopAll, iterCb);
    } else {
      eachLimit(appConf, conf.CONCURRENT_ACTIONS, iterForStopAll, iterCb);
    }
    ...
    function iterForStopAll(proc, next1) {
      var name = '';
      var new_env;
