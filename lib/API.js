    ...
    function processIds(ids, cb) {
      ...
      eachLimit(ids, concurrent_actions, function(id, next) {
        ...
        if (action_name == 'restartProcessId' ||
          ...
          if (update_env === true) {
            // Same rule as the initial start in Common.prepareAppConf: the
            // shell may not re-supply PM2 options on --update-env, only real
            // environment variables. The app's own config is layered on top.
        ...
        that.Client.executeRemote(action_name, opts, function(err, res) {
          if (err) {
            // Surface the daemon's actual error (e.g. `Reload already in
            // progress`) instead of masking everything as `not found`
            var error_msg = (err && err.message) ? err.message : `Process ${id} not found`;
            Common.printError(conf.PREFIX_MSG_ERR + '%s', error_msg);
