...
API.prototype._startScript = function(script, opts, cb) {
  ...
  var ignoreArgs = flagWatch.filterIgnoreArgs(opts && opts.rawArgs ? opts.rawArgs : []);
  var fileArray = [];
  ...
  if (ignoreArgs.length !== 0){
    for (var i = 0; i < ignoreArgs.length; i++) {
    	flagWatch.handleFolders(ignoreArgs[i], fileArray);
    }
  }
