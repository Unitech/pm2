...
////////////////////////////
  ...
  _startJson (file, opts, action, pipe, cb) {
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, raw_proc_list) {
      ...
      if (appConf.some(env => { if (env.priority) return true; })) {
        ...
        async.eachSeries(sortedKeys, iterFunctionForAsync, callbackAsync);        
      } else {
        async.eachLimit(Object.keys(proc_list), conf.CONCURRENT_ACTIONS, iterFunctionForAsync, callbackAsync);
      }
      return false;
    ...
    function startApps(app_name_to_start, cb) {
      ...
      if (apps_to_start.some(env => { if (env.priority) return true; })) {
        ...
        async.eachSeries(apps_to_start, iterForAll, asyncCB);
      } else {
        async.eachLimit(apps_to_start, conf.CONCURRENT_ACTIONS, iterForAll, asyncCB);
      }
      return false;
  ...
  actionFromJson (action, file, opts, jsonVia, cb) {
    ...
    if (appConf.some(env => { if (env.stop_priority) return true; }) || appConf.some(env => { if (env.priority) return true; })) {
      ...
      async.eachSeries(appConf, iterForStopAll, iterCb);
    } else {
      async.eachLimit(appConf, conf.CONCURRENT_ACTIONS, iterForStopAll, iterCb);
    }
