  ...
  function getConfig() {
    if (typeof(file) === 'object')
      ...
      return new Promise(function (resolve, reject) {
        ...
        Common.parseConfig(data, file).then(function (config) {
          resolve(config)
        }, function (e) {
...
API.prototype.actionFromJson = function(action, file, opts, jsonVia, cb) {
  ...
  getAppConf().then(function (appConf) {
  ...
  function getAppConf() {
    return new Promise(function (resolve, reject) {
      ...
      if (typeof file == 'object') {
        ...
        Common.parseConfig(data, file).then(function (appConf) {
          resolve(appConf)
        }, function(e) {
          Common.printError(conf.PREFIX_MSG_ERR + 'File ' + file + ' malformated');
          console.error(e);
        ...
        Common.parseConfig(file, 'pipe').then(function (config) {
          resolve(config)
        })
      } else {
        Common.printError('Bad call to actionFromJson, jsonVia should be one of file, pipe');
        reject('Bad call to actionFromJson, jsonVia should be one of file, pipe')
