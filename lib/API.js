...
////////////////////////////
  ...
  speedList (code, apps_acted) {
    ...
    if (apps_acted && apps_acted.length > 0)
      acted = apps_acted.reduce((acc, proc) => {
        if (!proc) return acc
        let app_name = proc.pm2_env ? proc.pm2_env.name : proc.name
        if (acc.includes(app_name) == false)
          acc.push(app_name)
        return acc
      }, [])
    ...
    function doList(err, list, sys_infos) {
      ...
      else if (acted.length > 0 && (process.stdout.isTTY || commander.attach === true)) {
