...
Satan.remoteWrapper = function() {
  ...
  server.expose({
    ...
    startDocker             : God.startDocker,
    restartDocker           : God.restartDocker,
    stopDocker              : God.stopDocker,
...
Satan.executeRemote = function executeRemote(method, env, fn) {
  ...
  //stop watch on stop (stop doesn't accept env, yet)
    ...
    Satan.stopWatch(method, env);
  //restart watch
  } else if ((env.watch == true || ~process.argv.indexOf('--watch')) && method.indexOf('restart') !== -1) {
