...
Satan.remoteWrapper = function() {
  ...
  this.rpc_socket = rep.bind(cst.DAEMON_RPC_PORT, cst.DAEMON_BIND_HOST);
  ...
  this.pub_socket = pub.bind(cst.DAEMON_PUB_PORT, cst.DAEMON_BIND_HOST);
...
Satan.disconnectRPC = function disconnectRPC(cb) {
  ...
  process.nextTick(function() {
    ...
    try {
      ...
      debug('Error while disconnecting RPC', e.stack || e);
      return cb ? cb(null, {success:true}) : false;
    };
    return false;
  });
