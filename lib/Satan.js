...
Satan.executeRemote = function(method, env, fn) {
  ...
  //Tricky methods to stop or restart the watcher
  if(process.argv.indexOf('--watch') !== -1) {
    if(method.indexOf('stop') !== -1) {    
      //stops watching
      Satan.client.call('stopWatch', env, method, fn)
    } else if(method.indexOf('restart') !== -1) {
      //toggles watching
      Satan.client.call('restartWatch', env, method, fn)
    }
  }
