...
Satan.remoteWrapper = function() {
  ...
  /**
   * Set option has enabled and ready
   */
  God.bus.on('axm:option:enable', function axmMonitor(msg) {
    var pm2_env = msg.process;

    if (!pm2_env || !God.clusters_db[pm2_env.pm_id])
      return console.error('[axm:option:enable] Unknown id %s', pm2_env.pm_id);

    if (!msg.name)
      return console.error('[axm:option:enable] Name not defined');

    God.clusters_db[pm2_env.pm_id].pm2_env.axm_options[msg.name] = true;
  ...
  God.bus.on('axm:monitor', function axmMonitor(msg) {
    ...
    God.clusters_db[pm2_env.pm_id].pm2_env.axm_monitor = msg.data;
  ...
  God.bus.onAny(function interactionBroadcast(data_v) {
    if (['axm:action', 'axm:monitor', 'axm:option:enable'].indexOf(this.event) > -1)
      return false;
