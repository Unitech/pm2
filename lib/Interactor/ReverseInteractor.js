  ...
  start : function(p) {
    ...
    this.socket.on('close', function(dt) {
      ...
      setTimeout(function() {
        ...
        self.socket.connect(self.network.hostname, self.network.port);
      }, 2000);
    });
    ...
    console.log('Connecting %s %s',this.network.port, this.network.hostname);
    this.socket.connect(this.network.hostname, this.network.port);
