...
var ReverseInteract = module.exports = {
  ...
  pm2Actions : function() {
    ...
    function executionBox(raw_msg) {
      var msg = {};

      // return self.socket.send('ok');
      ...
      if (!msg)
        return console.error('Wrong SECRET KEY Tto uncipher package');

      /**
       * Exemple
       * msg = {
       *   method_name : 'restart',
       *   parameters  : '<name|pm_id|all>'
       * }
       */
      console.log('PM2 action from remote triggered "pm2 %s %s"',
                  msg.method_name,
                  msg.parameters);
      ...
      if (!method_name)
        return console.error('no method name');

      if (PM2_REMOTE_METHOD_ALLOWED.indexOf(method_name) == -1)
        return console.error('method %s not allowed', method_name);

      pm2.connect(function() {
        pm2[method_name](parameters, function(err, ret) {
          self.socket.send('pm2_action:result', {
            err : err,
            ret : ret
          });
