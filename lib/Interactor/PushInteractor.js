...
var PushInteractor = module.exports = {
  ...
  start : function(p) {
    ...
    this.tcpSocket.on('connect', function() {
      console.log('[PUSH] Connected via TCP', cst.REMOTE_PORT_TCP);
    });
    ...
    this.preparePacket(function() {
      ...
      if (self.conf._connection_is_up === true) {
        ...
        var timer = setInterval(function() {
          console.error('[CRITICAL][FALLBACK] Clearing agent data cache to avoid leakage');
          self.resetPacket();
        }, 1000 * 60);
        ...
        self.tcpSocket.sendv2(str, function() {
          ...
          if (duration_sec > 1)
            console.error('[CRITICAL] Time to send data over TCP took %dseconds!', duration_sec);
        ...
        console.log('[CRITICAL] Connection is down');
        data = null;
        str  = null;
        return cb();
      }
