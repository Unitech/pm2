...
var PushInteractor = module.exports = {
  ...
  listenToPM2Events : function() {
    ...
    this.ipm2.bus.on('*', function(event, packet) {
      ...
      if (event.match(/^log:/)) {
        if (!self.log_buffer[packet.process.name])
          self.log_buffer[packet.process.name] = [];
        // push the log data
        self.log_buffer[packet.process.name].push(packet.data);
        // delete the last one if too long
        if (self.log_buffer[packet.process.name].length === 25)
          self.log_buffer[packet.process.name].pop();
      ...
      if (event === 'process:exception')
        packet.data.last_logs = self.log_buffer[packet.process.name];
