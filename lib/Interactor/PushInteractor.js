  ...
  listenToPM2Events : function() {
    ...
    this.ipm2.bus.on('*', function(event, packet) {
      ...
      if (event.match(/^log:/)) {
        ...
        if (self.log_buffer[packet.process.pm_id].length >= 25)
      ...
      if (event.indexOf('axm:trace') > -1 && packet.data)
        return self.aggregator.aggregate(packet.data);
