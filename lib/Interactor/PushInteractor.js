...
var PushInteractor = module.exports = {
  start : function(p) {
    ...
    this.ipm2.on('reconnecting', function() {
      ...
      if (self.timer_worker)
        clearInterval(self.timer_worker);
  ...
  startWorker : function() {
    ...
    self.timer_worker = setInterval(function() {
      self.sendData();
    }, cst.SEND_INTERVAL);
  },
  processEvents : function() {
    ...
    this.ipm2.bus.on('*', function(event, packet) {
      ...
      if (packet.process && packet.process.pm2_env) {
        ...
        packet.process = Filter.pruneProcessObj(packet.process, self.conf.MACHINE_NAME);
  ...
  bufferizeServerStatus : function(cb) {
    ...
    this.ipm2.rpc.getMonitorData({}, function(err, processes) {
      ...
      if ((ret = Filter.monitoring(processes, self.conf))) {
        self.bufferData('monitoring', ret);
      }

      if ((ret = Filter.status(processes, self.conf))) {
  ...
  bufferData : function(event, packet) {
    ...
    if (packet.process && !packet.server) {
      ...
      if (event == 'http:transaction') {
        packet.data.data.process_id = Filter.getProcessID(self.conf.MACHINE_NAME, packet.process.name, packet.process.pm_id);
      ...
      self.send_buffer.push({
        ...
        process_id : Filter.getProcessID(self.conf.MACHINE_NAME, packet.process.name, packet.process.pm_id),
