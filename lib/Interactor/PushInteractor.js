...
var PushInteractor = module.exports = {
  ...
  listenToPM2Events : function() {
    ...
    this.ipm2.bus.on('*', function(event, packet) {
      ...
      if (event.match(/^log:/)) {
        if (!self.log_buffer[packet.process.pm_id])
          self.log_buffer[packet.process.pm_id] = [];
        // push the log data
        self.log_buffer[packet.process.pm_id].push(packet.data);
        // delete the last one if too long
        if (self.log_buffer[packet.process.pm_id].length >= cst.LOGS_BUFFER)
          self.log_buffer[packet.process.pm_id].pop();
      ...
      if (event === 'process:exception') {
        packet.data.last_logs = self.log_buffer[packet.process.pm_id];

        // try to parse stacktrace and attach callsite + context if available
        if (typeof(packet.data.stackframes) === 'object') {
          var result = self.stackParser.parse(packet.data.stackframes);
  ...
  detectUnitMonitoring : function(processes) {
    var that = this;
    var new_process_list = [];

    that.monitored_processes = {};

    processes.forEach(function(proc) {
      if (proc && proc.pm2_env && proc.pm2_env._km_monitored === true) {
        that.monitored_processes[proc.pm2_env.pm_id] = true;
        new_process_list.push(proc);
      }
    });

    if (new_process_list.length > 0)
  ...
  preparePacket : function(cb) {
    ...
    this.ipm2.rpc.getMonitorData({}, function(err, processes) {
      ...
      processes = self.detectUnitMonitoring(processes);
