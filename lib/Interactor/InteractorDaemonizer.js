...
InteractorDaemonizer.daemonize = function(infos, cb) {
  ...
  child.once('message', function(msg) {
     ...
     */
    if (msg.error == true) {
      console.log(chalk.red('[Keymetrics.io][ERROR]'), msg);
    ...
    if (msg.km_data.error == true) {
      console.log(chalk.cyan('[Keymetrics.io]') + ' ' + msg.km_data.msg + ' (Public: %s) (Secret: %s) (Machine name: %s)', msg.public_key, msg.secret_key, msg.machine_name);
      return cb(msg);
    }
    ...
    // if (msg.km_data.active == false && msg.km_data.pending == false) {
    //   console.log(chalk.cyan('[Keymetrics.io]') + ' Server deactivated (not active and not pending) contact support contact@keymetrics.io with reference to your public and secret keys)');
    //   return cb(msg);
    // }
