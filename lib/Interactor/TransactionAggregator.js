...
var FLUSH_INTERVAL = process.env.NODE_ENV === 'local_test' || process.env.PM2_DEBUG ? 1000 : 2500;
...
var TransactionAggregator = module.exports = function (pushInteractor) {
  ...
  this.aggregate = function(packet) {
    ...
    // parse strackrace
    self.parseStackrace(new_trace.spans);
   ...
   * @param {Object} spans list of span for a trace
   */
  this.parseStackrace = function (spans) {
    if (!spans)
      return log('spans is null');

    spans.forEach(function (span) {
      // if empty make sure that it doesnt exist
      if (!span.labels.stacktrace || typeof(span.labels.stacktrace) !== 'string'|| span.labels.stackrace === '[]')
        return;
      ...
      var callsite = span.labels.stacktrace[0];
      if (!callsite)
        return ;
      span.labels['source/file'] =  { file: callsite.file_name + '',
          line: callsite.line_number + ''};

      if (callsite.context) {
        var tmp = [];
        if (callsite.context.pre && callsite.context.pre.length > 0) {
          // clean value from useless chars and push it
          callsite.context.pre.forEach(function (line) {
            tmp.push(line.replace(/\t/g, '  '));
          })
        }

        if (callsite.context.line)
          tmp.push(callsite.context.line.replace(/\t/g, '  ').replace('  ', '>>'))

        if (callsite.context.post && callsite.context.post.length > 0) {
          // clean value from useless chars and push it
          callsite.context.post.forEach(function (line) {
            tmp.push(line.replace(/\t/g, '  '));
          })
        }
        
        // only define it if we got something
        if (tmp.length > 0)
          span.labels['source/code'] = tmp.join('\n');
      }
    })
