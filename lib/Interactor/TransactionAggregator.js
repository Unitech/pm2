...
var TransactionAggregator = module.exports = function (pushInteractor) {
  ...
  this.aggregate = function (event, packet) {
    ...
    console.log(JSON.stringify(packet.data.traces))
    ...
    async.eachLimit(packet.data.traces, 1, function (trace, next) {
      ...
      self.convertSpanListToTree(trace, function (tree) {
        ...
        if (!trace.spans ||
            !trace.spans.kind !== 'RPC_SERVER' ||
            !trace.spans.labels ||
            trace.spans.labels.indexOf(LABELS.HTTP_PATH_LABEL_KEY) < 0)
          return next();
  ...
  this.mergeTrace = function (aggregated, trace, cb) {
    ...
    var merge = function (variance) {
      ...
      // variance found, merge spans
        ...
        variance.min = variance.min > trace.spans.duration ? trace.spans.duration : variance.min;
        variance.max = variance.max < trace.spans.duration ? trace.spans.duration : variance.max;
        variance.mean = (trace.spans.duration + (variance.mean * variance.count)) / (variance.count + 1);
        
        // update duration of spans to be mean
        self.updateMeanDuration(variance.spans, trace.spans, variance.count, true);
        variance.count++;
      }
      return cb();
    }
  ...
  this.computeSpanDuration = function (head) {
    // head
    if (head.parentSpanId === 0 || head.parentSpanId === "0") {
      if (head.endTime && head.startTime)
        head.duration = new Date(head.endTime) - new Date(head.startTime);
      delete head.endTime;
      delete head.startTime;
      delete head.stackrace;
    }
    // childs
    if (head.child)
      head.child.forEach(function(span) {
        if (span.endTime && span.startTime)
          span.duration = new Date(span.endTime) - new Date(span.startTime);
        delete span.endTime;
        delete span.startTime;
        delete span.stackrace;
        if (span.child)
  ...
  setInterval(function () {
    console.log(JSON.stringify(self.processes, null, 2))
  }, 5000);
};
