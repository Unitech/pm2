 ...
 *      ],
 *      meta : {
 *        transaction_count : 50,  // trace number
...
var TransactionAggregator = module.exports = function (pushInteractor) {
  ...
  setInterval(function () {
    ...
    async.forEachOf(self.processes, function (process, name, next) {
      ...
      normalized[name] = {
        data: {
          ...
          meta: JSON.parse(JSON.stringify({
            transaction_count: process.meta.transaction_count,
      ...
      // for every route
      async.forEachOf(routes, function (data, route, next2) {
        // get top 5 variances of the same route
        variances = data.variances.sort(function (a, b) {
        ...
        variances.forEach(function (variance) {
          ...
          var tmp = JSON.parse(JSON.stringify({
        ...
        normalized[name].data.routes.push(routeCopy);
        return next2();
      }, next);
    }, function () {
  ...
  this.aggregate = function (event, packet) {
    ...
    trace.spans.forEach(function (span) {
      ...
      // update http latency/meter
        ...
        process.meta.mean_latency = process.meta.transaction_count > 0 ?
          (duration + (process.meta.mean_latency * process.meta.transaction_count)) / (process.meta.transaction_count + 1) : duration;
  ...
  this.mergeTrace = function (aggregated, trace) {
    ...
    if (!aggregated || !aggregated.variances)
      return;
