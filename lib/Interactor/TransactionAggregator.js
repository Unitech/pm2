...
var SPANS_DB = ['redis', 'mysql', 'pg', 'mongodb'];
...
var TransactionAggregator = module.exports = function (pushInteractor) {
    ...
    var merge = function (variance) {
      ...
      if (variance == null) {
        ...
        if (trace.spans[0].mean > variance.histogram.percentiles([0.95])[0.95]) {
          ...
          pushInteractor.bufferData('axm:transaction:inquisitor', data);
        }
  ...
  this.updateSpanDuration = function (ref_spans, spans, count) {
    for (var i = 0, len = ref_spans.length; i < len; i++) {
      ref_spans[i].histogram.update(spans[i].mean);
    }
  };
      ...
      normalized[app_name] = {
        data: {
          ...
          meta: fclone({
            ...
            http_percentiles: process.meta.histogram.percentiles([0.5, 0.95, 0.99]),
            db_percentiles: {}
          })
        },
