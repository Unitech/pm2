...
var Client = module.exports = function(opts) {
  ...
  this.daemon_mode = typeof(opts.daemon_mode) == 'undefined' || true;
...
Client.prototype.start = function(cb) {
  ...
  this.pingDaemon(function(daemonAlive) {
    ...
    that.launchDaemon(function(err, child) {
      ...
      that.printOut(that.conf.PREFIX_MSG + 'PM2 Successfully daemonized');
      that.launchRPC(function(err, meta) {
        return cb(null, {
          daemon_mode      : that.conf.daemon_mode,
...
Client.prototype.initFileStructure = function(opts) {
  ...
  if (!fs.existsSync(opts.PM2_ROOT_PATH)) {
    fs.mkdirSync(opts.PM2_ROOT_PATH);
    fs.mkdirSync(opts.DEFAULT_LOG_PATH);
    fs.mkdirSync(opts.DEFAULT_PID_PATH);
  }
  ...
  if (opts.PM2_HOME && !fs.existsSync(opts.PM2_HOME)) {
    try {
      fs.mkdirSync(opts.PM2_HOME);
      fs.mkdirSync(opts.DEFAULT_LOG_PATH);
      fs.mkdirSync(opts.DEFAULT_PID_PATH);
    } catch(e) {
      debug(e.stack || e);
...
Client.prototype.launchDaemon = function(cb) {
  ...
  that.printOut(that.conf.PREFIX_MSG + 'Spawning PM2 daemon with pm2_home=' + this.pm2_home);
  ...
  child.once('message', function(msg) {
     ...
     */
    InteractorDaemonizer.launchAndInteract(that.conf, {
...
Client.prototype.printOut = function() {
  //if (process.env.NODE_ENV == 'test' && (process.env.PM2_SILENT || process.env.PM2_PROGRAMMATIC === 'true')) return false
