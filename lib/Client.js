  ...
  client.sock.once('error', function(e) {
    if (e.code == 'EACCES') {
      fs.stat(that.conf.DAEMON_RPC_PORT, function(e, stats) {
        if (stats.uid === 0) {
          console.error(that.conf.PREFIX_MSG_ERR + 'Permission denied, command needs to be runned as root or set the right permission via chown');
          console.error(that.conf.PREFIX_MSG_ERR + 'Files that needs permissions:', that.conf.DAEMON_RPC_PORT, 'and', that.conf.DAEMON_PUB_PORT);
        }
        else
