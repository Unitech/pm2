...
module.exports = function(CLI) {
  ...
  CLI.prototype.resurrect = function(cb) {
    ...
    (function ex(apps) {
      if (!apps[0]) return cb ? cb(null, apps) : that.speedList();
      that.Client.executeRemote('prepare', apps[0], function(err, dt) {
        if (err)
        ...
        that.Client.notifyGod('resurrect', dt[0].pm2_env.pm_id);

        apps.shift();
        return ex(apps);
      });
      return false;
    })(Common.parseConfig(apps, 'none'));
  };
