...
UX.describeTable = function(process) {
  ...
  console.log(chalk.inverse.bold(' Describing process with id %d - name %s '), pm2_env.pm_id, pm2_env.name);
  safe_push(table,
    ...
    { 'args' : pm2_env.args ? (typeof pm2_env.args == 'string' ? JSON.parse(pm2_env.args.replace(/'/g, '"')):pm2_env.args).join(' ') : null },
    ...
    { 'node v8 arguments' : pm2_env.node_args.length != 0 ? pm2_env.node_args : null },
