  ...
  CLI.logrotate = function(opts, cb) {
    if (process.getuid() != 0) {
      return exec('whoami', function(err, stdout, stderr) {
        ...
        cb ? cb(Common.retErr('You have to run this with elevated rights')) : Common.exitCli(cst.ERROR_EXIT);
      });
    }
    ...
    if (!fs.existsSync('/etc/logrotate.d')) {
      ...
      return cb ? cb(Common.retErr('/etc/logrotate.d does not exist')) : Common.exitCli(cst.ERROR_EXIT);
    }
