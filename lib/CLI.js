...
CLI.streamLogs = function(id, lines, raw, timestamp) {
  var files_list = [];
  ...
  Satan.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    if (!raw)
      printOut('[PM2] Tailing last %d lines for [%s] process%s', lines, id, id === 'all' ? 'es' : '', '\n');
    ...
    if (!raw && (id === 'all' || id === 'PM2')) {
      Log.tail([{
        ...
        Log.tail(files_list, lines, raw, function() {
          printOut('[PM2] Streaming realtime logs for [%s] process%s', id, id === 'all' ? 'es' : '', '\n');
          Log.stream(id, raw, timestamp);
        });
      });
    }
    else {
