...
CLI.start = function(script, opts, cb) {
  ...
  if (appConf['exec_mode'] == 'cluster_mode' && process.version.match(/0.10/)) {
    printOut(cst.PREFIX_MSG_ERR + ' [Warning], you\'re using the 0.10.x node version, it\'s prefered that you switch to fork mode by adding the -x parameter. 0.11.x can use cluster_mode (default mode) efficiently.');
  }
...
CLI.updatePM2 = function(cb) {
  ...
  InteractorDaemonizer.kill(function() {
  ...
  CLI.dump(function(err) {
    ...
    CLI.killDaemon(function(err) {
      ...
      Satan.launchDaemon(function(err, child) {
        printOut(cst.PREFIX_MSG + '--- resurrecting');
        if (err) {
          printError(err);
          return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
        }
