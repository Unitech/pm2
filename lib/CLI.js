...
function resolvePaths(app) {
  ...
  if (app.exec_mode == 'fork_mode') {
    // It's binary/command
    var cmd = app.script.split(' ');
    var tmp;

    app["pm_exec_path"] = "";
    
    cmd.forEach(function(part) {
      if (fs.existsSync(part) == true) {
        tmp = path.resolve(process.cwd(), part);
      }
      else
        tmp = part;

      app["pm_exec_path"] += tmp + ' ';
    });
  }
  else {
    // It's a node app or binary
