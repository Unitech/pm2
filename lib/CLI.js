...
CLI.startFile = function(script) {
  ...
  Satan.executeRemote('findByScript', p.basename(script), function(err, exec) {
...
CLI.startFromJson = function(cmd) {
  ...
  if (Array.isArray(appConf)) {
    ...
    Satan.executeRemote('findByScript', p.basename(appConf.script), function(err, exec) {
      if (exec && !commander.force) {
...
CLI.deleteProcess = function(process_name) {
  if (process_name == 'all') {
    ...
    Satan.executeRemote('deleteProcess', process_name, function(err, list) {
      if (err) {
        console.error('\n' + cst.PREFIX_MSG_ERR + err);
        process.exit(cst.ERROR_EXIT);
      }
...
function resolvePaths(app) {
  ...
  // if there is no space in the passed command
  // we dont resolve the command
  if (app.script.split(' ').length == 1) {
