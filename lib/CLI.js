...
CLI._startJson = function(cmd, opts, jsonVia, cb) {
  ...
  function startApps(app_name_to_start, cb) {
    ...
    async.eachLimit(apps_to_start, cst.CONCURRENT_ACTIONS, function(app, next) {
      ...
      Satan.executeRemote('prepare', resolved_paths, function(err, data) {
        if (err) {
          printError(cst.PREFIX_MSG + 'Process failed to launch', err);
          return next();
        }
...
CLI.deploy = function(file, commands, cb) {
  ...
  try {
      json_conf = parseConfig(fs.readFileSync(file), file);
  } catch (e) {
    printError(e);
    return cb ? cb(e) : exitCli(cst.ERROR_EXIT);
  }
  ...
  if (!env) {
    if (Object.keys(json_conf.deploy).length == 1) {
      ...
      deployHelp();
      return cb ? cb('Configuration file has more than 1 environment. Needs to specify which one.') : exitCli(cst.SUCCESS_EXIT);
    }
  }
 ...
 * @return
 */
CLI.sendDataToProcessId = function(packet, cb) {
  Satan.executeRemote('sendDataToProcessId', packet, function(err, res) {
    if (err) {
      printError(err);
      return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
    }
    printOut('successfully sent data to process');
...
function deployHelp() {
  // Don't show help if called programmatically (while running a test)
