...
CLI._startScript = function(script, opts, cb) {
   ...
   */
  if (appConf.write) {
    var dst_path = path.join(cst.PWD, conf.name + '-pm2.json');
...
CLI.generateSample = function() {
  ...
  try {
    fs.writeFileSync(path.join(cst.PWD, f_name), dt);
  } catch (e) {
    console.error(e.stack || e);
  }
  Common.printOut('File %s generated', path.join(cst.PWD, f_name));
