...
CLI.deepUpdate = function(cb) {
  ...
  exec("npm i -g pm2@latest", function(err, stdout, stderr) {
    if (err)
      return cb ? cb(err) : printError(cst.PREFIX_MSG_ERR + (err.stack || err));

    printOut(stdout);
    printError(stderr);

    exec("pm2 update", function(err, stdout, stderr) {
      if (err)
        return cb ? cb(err) : printError(cst.PREFIX_MSG_ERR + (err.stack || err));
