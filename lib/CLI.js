...
CLI.start = function(cmd, opts, cb) {
  ...
  if (opts.docker === true) {
    Satan.executeRemote('startDocker', cmd, function(err, data) {
      if (err) {
        printError(cst.PREFIX_MSG_ERR + err);
        return exitCli(cst.ERROR_EXIT);
      }
      printOut(cst.PREFIX_MSG + 'Container ' + cmd + ' started.');
      return speedList();
    });
  }
  else if ((typeof(cmd) === 'string' && cmd.indexOf('.json') != -1) || typeof(cmd) === 'object')
  ...
  async.eachLimit(appConf, cst.CONCURRENT_ACTIONS, function(proc, next1) {
    ...
    Common.getProcessIdByName(name, function(err, ids) {
      ...
      async.eachLimit(ids, cst.CONCURRENT_ACTIONS, function(id, next2) {
        ...
        opts = util._extend(opts, {watch: proc.watch});
  ...
  function processIds(ids, cb) {
    async.eachLimit(ids, cst.CONCURRENT_ACTIONS, function(id, next) {
      ...
      Satan.executeRemote(action_name, opts, function(err, res) {
        if (err) {
          if (err.message && err.message.indexOf('You must use --docker') != -1)
            printError(cst.PREFIX_MSG_ERR + err);
          else
...
CLI.restart = function(process_name, commander, cb) {
...
CLI.stop = function(process_name, commander, cb) {
  if (typeof(process_name) === 'number')
    process_name = process_name.toString();
  ...
  if (commander.docker === true) {
    Satan.executeRemote('stopDocker', process_name, function(err, data) {
      if (err) {
        printError(cst.PREFIX_MSG_ERR + err);
        return exitCli(cst.ERROR_EXIT);
      }
      printOut(cst.PREFIX_MSG + 'Container ' + process_name + ' stopped');
      return speedList();
    });
