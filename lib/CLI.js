 ...
 * @return
 */
CLI.startFile = function(script, cb) { var appConf = {
    script : script,
  ...
  Satan.executeRemote('findByFullPath', path.resolve(process.cwd(), script), function(err, exec) {
    if (exec &&
      ...
      Satan.executeRemote('restartProcessName', app_name, function(err, list) {
        ...
        if (cb) return cb(null, {data : list});
...
CLI.connect = function(cb) { Satan.start(cb);
};

CLI.disconnect = function(cb) { Satan.disconnectRPC(cb);
};
 ...
 * @callback cb
 */
CLI.getVersion = function(cb) { Satan.executeRemote('getVersion', {}, function(err, version) {
...
CLI.actionFromJson = function(action, file, jsonVia, cb) { var appConf;
  ...
  async.eachLimit(appConf, cst.CONCURRENT_ACTIONS, function(proc, next) {
    ...
    if (cb) return cb(null, {data : {success:true}});
 ...
 */
CLI.startFromJson = function(cmd, jsonVia, cb) { var appConf;
 ...
 */
CLI.interact = function(secret_key, public_key, machine_name, cb) { InteractorDaemonizer.launchOrAttach(secret_key, public_key, machine_name, function(status) {
    if (status == false) {
 ...
 * @method killInteract
 */
CLI.killInteract = function(cb) { InteractorDaemonizer.ping(function(online) {
    if (!online) {
 ...
 * @method infoInteract
 */
CLI.infoInteract = function(cb) { getInteractInfo(function(err, data) {
    if (err) {
      printError('Interactor not launched');
      return cb ? cb({msg:'Interactor not launched'}) : exitCli(cst.ERROR_EXIT);
    }
    printOut(data);
    return cb ? cb(null, {data:data}) : exitCli(cst.SUCCESS_EXIT);
  });
};
 ...
 * @method ping
 */
CLI.ping = function(cb) { Satan.executeRemote('ping', {}, function(err, res) {
    if (err) {
      printError(err);
      return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
    }
...
CLI.resurrect = function(cb) { try {
  ...
  (function ex(apps) {
    if (!apps[0]) return cb ? cb(null, {data:apps}) : speedList();
 ...
 * @return
 */
CLI.updatePM2 = function(cb) { printOut('Be sure to haave the latest version by doing `npm install pm2@latest -g` before doing this procedure.');
    ...
    CLI.killDaemon(function(err) {
      ...
      Satan.launchDaemon(function(err, child) {
        ...
        CLI.resurrect(function() {
          ...
          return cb ? cb(null, {data:{success:true}}) : speedList();
        });
        return false;
      });
    });
 ...
 * @return
 */
CLI.dump = function(cb) { var env_arr = [];
  Satan.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    function fin(err) {
      ...
      UX.processing.stop();
      if (cb) return cb(null, {msg:{success:true}});
 ...
 * @return
 */
CLI.web = function(cb) { Satan.executeRemote('prepare', resolvePaths({
    ...
    printOut(cst.PREFIX_MSG + 'Process launched');
    return cb ? cb(null, {data:proc}) : speedList();
  });
};
 ...
 * @return
 */
CLI.reload = function (reload_method, cb) { Satan.executeRemote('getMonitorData', {}, function(err, list) {
    if (err) {
      printError('Error retrieving process list: ' + err);
      return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
    }
    ...
    async.eachLimit(list, 1, function(proc, next) {
      ...
      return cb ? cb(null, {data:list}) : setTimeout(speedList, 500);
    });
    return false;
  });
};
 ...
 * @return
 */
CLI.reloadProcessName = function (process_name, reload_method, cb) { printOut(cst.PREFIX_MSG + 'Reloading process by name %s', process_name);
  ...
  getProcessByName(process_name, function(err, processes) {
    ...
    async.eachLimit(processes, 1, function(proc, next) {
      ...
      printOut(cst.PREFIX_MSG + 'All processes reloaded');
      return cb ? cb(null, {data:processes}) : setTimeout(speedList, 500);
    });
    return false;
  });
};
 ...
 * @return
 */
CLI.restartProcessByName = function(pm2_name, cb) { Satan.executeRemote('restartProcessName', pm2_name, function(err, list) {
    if (err) {
      printError(err);
      return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
    }
    ...
    return cb ? cb(null, {data:list}) : speedList();
  });
};
 ...
 * @return
 */
CLI.restartProcessById = function(pm2_id, cb) { Satan.executeRemote('restartProcessId', pm2_id, function(err, res) {
    if (err) {
      printError(err);
      return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
    }
    ...
    return cb ? cb(null, {data:res}) : speedList();
  });
};
 ...
 * @return
 */
CLI.restartAll = function(cb) { Satan.executeRemote('getMonitorData', {}, function(err, list) {
    if (err) {
      printError(err);
      return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
    }
    ...
    (function rec(processes) {
      ...
      if (proc == null) {
        ...
        return cb ? cb(null, {data:processes}) : setTimeout(speedList, 1000);
      }
 ...
 * @return
 */
CLI.stopAll = function(cb) { Satan.executeRemote('stopAll', {}, function(err, list) {
    if (err) {
      printError(err);
      return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
    }
 ...
 * @return
 */
CLI.deleteProcess = function(process_name, jsonVia, cb) { if (typeof(jsonVia) == "function") {
    cb = jsonVia;
    jsonVia = null;
  }
 ...
 * @return
 */
CLI.stopProcessName = function(name, cb) { Satan.executeRemote('stopProcessName', name, function(err, list) {
    if (err) {
      printError(err);
      return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
    }
 ...
 * @return
 */
CLI.stopId = function(pm2_id, cb) { Satan.executeRemote('stopProcessId', pm2_id, function(err, list) {
    if (err) {
      printError(err);
      return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
    }
...
CLI.flush = function(cb) { printOut(cst.PREFIX_MSG + 'Flushing ' + cst.PM2_LOG_FILE_PATH);
  ...
  Satan.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    return cb ? cb(null, {data:list}) : exitCli(cst.SUCCESS_EXIT);
  });
};
 ...
 * @return
 */
CLI.describeProcess = function(pm2_id, cb) { var found_proc = false;
  ...
  Satan.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    if (found_proc === false) {
      ...
      return cb ? cb(null, {data:'Process not found'}) : exitCli(cst.ERROR_EXIT);
    }

    if (!cb)
      UX.describeTable(found_proc);

    return cb ? cb(null, {data:found_proc}) : exitCli(cst.SUCCESS_EXIT);
  });
};
...
CLI.reloadLogs = function(cb) { printOut('Reloading all logs...');
  Satan.executeRemote('reloadLogs', {}, function(err, logs) {
    ...
    return cb ? cb(null, {data:logs}) : exitCli(cst.SUCCESS_EXIT);
  });
};
 ...
 * @return
 */
CLI.killDaemon = function(cb) { printOut(cst.PREFIX_MSG + 'Killing pm2...');
  ...
  Satan.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    async.eachLimit(list, 2, function(proc, next) {
      ...
      Satan.killDaemon(function(err, res) {
        ...
        return cb ? cb(null, {data:res}) : exitCli(cst.SUCCESS_EXIT);
      });
    });
    return false;
  });
