...
CLI.generalReload = function(process_name, cb, id, action) {
  if (process_name == 'all')
    CLI._reloadAll(id, cb);
  else if (process_name.indexOf('.json') > 0)
    CLI._jsonStartOrAction(action, process_name, commander);
  else
    CLI._reloadProcessName(process_name, id, cb);
};

CLI.gracefulReload = function(process_name, cb) {
  CLI.generalReload(process_name, cb, 'softReloadProcessId', 'gracefulReload');
};

CLI.reload = function(process_name, cb) {
  CLI.generalReload(process_name, cb, 'reloadProcessId', 'reload');
};
