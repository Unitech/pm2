...
CLI.pullAndRestart = function (process_name, cb) {
  CLI._pull({process_name: process_name, action: 'restart'}, cb);
}
...
CLI.pullAndReload = function (process_name, cb) {
  CLI._pull({process_name: process_name, action: 'reload'}, cb);
}
...
CLI.pullAndGracefulReload = function (process_name, cb) {
  CLI._pull({process_name: process_name, action: 'gracefulReload'}, cb);
}
      ...
      function(err, meta) {
        ...
        if (meta.success === true) {
          ...
          function (command_list) {
            exec("cd '"+proc.pm2_env.versioning.repo_path+"'" + command_list,
 ...
 * @return
 */
CLI.forward = function (process_name, cb) {
    ...
    if (proc.pm2_env.versioning) {
      ...
      function(err, meta) {
        ...
        if (meta.success === true) {
          ...
          function (command_list) {
            var is_ok = true;
            var a = exec("cd '"+proc.pm2_env.versioning.repo_path+"'" + command_list,
                 function(err, stdout, stderr) {
                   if (err !== null)
                   {
                     is_ok = false;
 ...
 * @return
 */
CLI.backward = function (process_name, cb) {
  ...
  Common.getProcessByName(process_name, function(err, processes) {
    ...
    var proc = processes[0];
    if (proc.pm2_env.versioning) {
      vizion.prev({folder: proc.pm2_env.versioning.repo_path},
      function(err, meta) {
        if (err !== null)
          return cb ? cb({msg:err}) : exitCli(cst.ERROR_EXIT);
        if (meta.success === true) {
          ...
          function (command_list) {
            exec("cd '"+proc.pm2_env.versioning.repo_path+"'" + command_list,
                 function(err, stdout, stderr) {
                   ...
                   printOut(cst.PREFIX_MSG + 'Process successfully downgraded %s', process_name);
                   printOut(cst.PREFIX_MSG + 'Current commit %s', meta.current_revision);
                   return CLI.gracefulReload(process_name, cb);
                 });
          });
 ...
 * @return
 */
function getPostUpdateCmds(repo_path, proc_name, cb) {
  if (repo_path[repo_path.length - 1] !== '/')
    repo_path += '/';

  fs.exists(repo_path+'process.json', function(exists){
    if (exists) {
      var data = require(repo_path+'process.json');
      if (data && data.apps) {
        data.apps.forEach(function(item){
          if (item.name && item.name === proc_name)
            return cb(item.post_update ? ';' + item.post_update : ';');
        });
      }
      return cb(';');
    }
