...
CLI.prototype._startScript = function(script, opts, cb) {
  ...
  if ((appConf = Common.verifyConfs(conf)) instanceof Error)
    return cb ? cb(Common.retErr(appConf)) : that.exitCli(cst.ERROR_EXIT);
  ...
  function restartExistingProcessPath(cb) {
    that.Client.executeRemote('findByFullPath', path.resolve(process.cwd(), script), function(err, exec) {
      if (err) return cb ? cb(new Error(err)) : that.exitCli(cst.ERROR_EXIT);
...
CLI.prototype._startJson = function(file, opts, action, pipe, cb) {
  ...
  if (typeof(file) === 'object')
    ...
    try {
      ...
      return cb ? cb(Common.retErr(e)) : that.exitCli(cst.ERROR_EXIT);
    }
  ...
  if ((appConf = Common.verifyConfs(appConf)) instanceof Error)
    return cb ? cb(appConf) : that.exitCli(cst.ERROR_EXIT);
  ...
  if (process_name == 'all') {
    ...
    that.Client.getProcessIdByName(process_name, function(err, ids) {
      ...
      if (ids.length === 0) {
        ...
        return cb ? cb(new Error('Unknown process name')) : that.exitCli(cst.ERROR_EXIT);
      }
      return processIds(ids, cb);
    });
  } else {
  ...
  if (process_name == 'all') {
    that.Client.getAllProcessId(function(err, ids) {
      ...
      if (!ids || ids.length === 0) {
        ...
        return cb ? cb(new Error('process name not found')) : that.exitCli(cst.ERROR_EXIT);
      }
...
CLI.prototype.getProcessIdByName = function(name, cb) {
  ...
  this.Client.getProcessIdByName(name, function(err, id) {
    ...
    return cb ? cb(null, id) : that.exitCli(cst.SUCCESS_EXIT);
  });
};
...
CLI.prototype.speedList = function(code) {
  ...
  that.Client.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    if (that.Client.daemon_mode == false) {
      ...
      return that.exitCli(code ? code : cst.SUCCESS_EXIT);
    }
  });
}
...
CLI.prototype.scale = function(app_name, number, cb) {
  ...
  this.Client.getProcessByName(app_name, function(err, procs) {
    ...
    if (!procs || procs.length === 0) {
      ...
      return cb ? cb(new Error('App not found')) : that.exitCli(cst.ERROR_EXIT);
    }
    ...
    if (procs[0].pm2_env.exec_mode !== 'cluster_mode') {
      ...
      return cb ? cb(new Error('App not in cluster mode')) : that.exitCli(cst.ERROR_EXIT);
    }
    ...
    if (typeof(number) === 'string' && number.indexOf('+') >= 0) {
      ...
      if (number < 0)
        ...
        return cb ? cb(new Error('Same process number')) : that.exitCli(cst.ERROR_EXIT);
      }
    }
  });
};
...
CLI.prototype.describe = function(pm2_id, cb) {
  ...
  that.Client.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    if (found_proc.length === 0) {
      ...
      return cb ? cb(null, []) : that.exitCli(cst.ERROR_EXIT);
    }
    ...
    return cb ? cb(null, found_proc) : that.exitCli(cst.SUCCESS_EXIT);
  });
};
