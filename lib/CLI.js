...
var Modularizer = require('./CLI/Modules/Modularizer.js');
 ...
 * @param {Boolean} [opts.independant=false]     unique PM2 instance (random pm2_home)
 * @param {Boolean} [opts.daemon_mode=true]      should be called in the same process or not
 * @param {String}  opts.public_key              keymetrics bucket public key
 * @param {String}  opts.secret_key              keymetrics bucket secret key
 * @param {String}  opts.machine_name            keymetrics instance name
 */
var CLI = module.exports = function(opts) {
   ...
   * PM2 HOME resolution
   */
  if (opts.pm2_home && opts.independant == true)
    throw new Error('You cannot set a pm2_home and independant instance in same time');
  ...
  KMDaemon.getInteractInfo(this.conf, function(i_err, interact) {
 ...
 */
CLI.prototype.connect = CLI.prototype.start = function(cb) {
  var that = this;
...
CLI.prototype.destroy = function(cb) {
  ...
  this.killDaemon(function() {
    ...
    fs.exists(test_path, function(err) {
      if (err) return cb(err);
      debug('Deleting temporary folder %s', that.pm2_home);
      exec(cmd, cb);
    });
...
CLI.prototype.disconnect = function(cb) {
  var that = this;
...
CLI.prototype.launchBus = function(cb) {
  this.Client.launchBus(cb);
};

/**
 * Exit methods for CLI
...
//////////////////////////
// Load all CLI methods //
//////////////////////////

require('./CLI/Deploy.js')(CLI);
require('./CLI/Modules/Modules.js')(CLI);
...
CLI.prototype._startScript = function(script, opts, cb) {
  ...
  function restartExistingProcessPath(cb) {
    that.Client.executeRemote('findByFullPath', path.resolve(that.cwd, script), function(err, exec) {
      if (err) return cb ? cb(new Error(err)) : that.exitCli(conf.ERROR_EXIT);

      if (exec && (exec[0].pm2_env.status == conf.STOPPED_STATUS ||
                   exec[0].pm2_env.status == conf.STOPPING_STATUS ||
                   exec[0].pm2_env.status == conf.ERRORED_STATUS)) {
      ...
      try {
        resolved_paths = Common.resolvePaths({
          cwd      : that.cwd,
          pm2_home : that.pm2_home
        }, app_conf);
      } catch(e) {
      ...
      util._extend(resolved_paths.env, {
        PM2_HOME : that.pm2_home
      });
