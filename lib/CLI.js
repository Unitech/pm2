...
CLI.start = function(script, opts, cb) {
  ...
  if (forceForkMode) {
    if(/^cluster(_mode)?$/i.test(conf.exec_mode)){
      warn('We\'ve notice that you are passing ' + chalk.blue('-x, --execute-command') + ' option, but the exec_mode is set to ' +
            chalk.blue(conf.exec_mode) + ', it has no effect and will be overrode as ' + chalk.blue('fork | fork_mode') + '.');
    }
    conf.exec_mode = 'fork_mode';
  }

  prepareInterpreter(conf);

  var ret = Config.verifyJSON(conf);
  ...
  var appConf = ret.config;

  if (appConf.write) {
    var dst_path = path.join(process.env.PWD, appConf.name + '-pm2.json');
