...
CLI.startup = function(platform, opts, cb) {
  ...
  if (platform == 'linux'){
    console.log(cst.PREFIX_MSG + 'Attempting to determine startup type.');
    try {
      var osRelease = ini.parse(fs.readFileSync('/etc/sos-release', 'utf-8'));
      switch(osRelease.ID) {
      case 'centos':
        platform = osRelease.VERSION_ID >= 7 ? 'systemd' : 'centos';
      }
    }
    catch (e) {
      console.log(cst.PREFIX_MSG_WARN + 'Could not determine startup type.  Using default.');
    }
  }
  ...
  var userHome = execSync('echo -n ~' + user).stdout;

  var pidFile = p.resolve(userHome, '.pm2/pm2.pid');
  ...
  switch (platform) {
    case 'systemd':
      cmd = 'su -l ' + user + ' -c "' + userCmd + '"; su -c "' + cmd + '"';
      break;
    case 'darwin':
      cmd = 'pm2 dump';
      break;
...
function checkDeprecates(conf){
  ...
  if (conf.instances && isNaN(conf.instances))
    conf.instances = 0;
}
