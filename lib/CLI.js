...
CLI.start = function(script, opts, cb) {
  ...
  if (!!opts.executeCommand)
    conf.exec_mode = 'fork';
  else if (typeof(opts.instances) !== 'undefined')
...
function checkExecMode(conf) {
  ...
  if (!isNaN(conf.instances) &&
      conf.instances  &&
      /^fork(_mode)?$/i.test(conf.exec_mode)) {

    warn('You shouldn\'t run the process in a \'' + chalk.blue(conf.exec_mode) + '\' mode with ' + chalk.blue(conf.instances) +' instance(s), ' +
         'we recommend using ' + chalk.blue('cluster | cluster_mode') + ' instead to take advantage of multi-core systems, and we\'ve amended it yet.');
  ...
  // if (/^fork(_mode)?$/i.test(conf.exec_mode)) {
  //   conf.exec_mode
  // }
  //if (
  // if (/^fork(_mode)?$/i.test(conf.exec_mode) &&
  //     /^cluster(_mode)?$/i.test(conf.exec_mode)) {

  //     warn('We\'ve notice that you are passing ' + chalk.blue('-x, --execute-command') + ' option, but the exec_mode is set to ' +
  //          chalk.blue(conf.exec_mode) + ', it has no effect and will be override as ' + chalk.blue('fork | fork_mode') + '.');
  //   }
  //   conf.exec_mode = 'fork_mode';
  // }
}
...
function prepareInterpreter(conf){
  ...
  // check exec_mode by the way.
  if (/^cluster(_mode)?$/i.test(conf.exec_mode) && process.version.match(/0.10/) && !process.env.TRAVIS) {
    warn('You shouldn\'t use the cluster_mode with node 0.10.x. Instead use fork mode with -x');
  }
}
/**
 * Show warnings
