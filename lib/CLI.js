...
CLI._startJson = function(cmd, opts, jsonVia, cb) {
  ...
  if (typeof(cmd) === 'object')
    ...
    appConf = Utility.parseConfig(cmd, 'none');
  else {
    var data = null;
    try {
  ...
  Satan.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    async.eachLimit(list, cst.CONCURRENT_ACTIONS, function(proc, next) {
      ...
      if (action == 'reload') {
        CLI._reloadProcessName(proc.pm2_env.name, 'reloadProcessId', opts, function(err, ret) {
        ...
        CLI._reloadProcessName(proc.pm2_env.name, 'softReloadProcessId', opts, function(err, ret) {
          if (err) Common.printError(err);
          // And Remove from array to spy
...
CLI.gracefulReload = function(process_name, opts, cb) {
  ...
  if (process_name == 'all')
    ...
    CLI._reloadProcessName(process_name, 'softReloadProcessId', opts, cb);
}
...
CLI.reload = function(process_name, opts, cb) {
  ...
  if (process_name == 'all')
    ...
    CLI._reloadProcessName(process_name, 'reloadProcessId', opts, cb);
}
...
CLI._reloadAll = function (reload_method, cb) {
  Common.getAllProcess(function(err, procs) {
    ...
    async.eachLimit(procs, cst.CONCURRENT_ACTIONS, function(proc, next) {
      ...
      Satan.executeRemote(reload_method, { id : proc.pm2_env.pm_id }, function(err, list) {
 ...
 * @return
 */
CLI._reloadProcessName = function(process_name, reload_method, opts, cb) {
  Common.printOut(cst.PREFIX_MSG + 'Reloading process by name %s', process_name);

  var envs = CLI._handleAttributeUpdate(opts);
  ...
  Common.getProcessByName(process_name, function(err, processes) {
    ...
    async.eachLimit(processes, cst.CONCURRENT_ACTIONS, function(proc, next) {
      ...
      if (proc.pm2_env.exec_mode != 'cluster_mode') {
        ...
        return CLI.restart(process_name, opts, next);
      }
      ...
      Satan.executeRemote(reload_method, args, function(err, res) {
        if (err)
          return next(err);
