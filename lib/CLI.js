...
CLI.reload = function (cluster) {
  console.log(cst.PREFIX_MSG + 'Reloading all processes');
...
CLI.reloadProcessName = function (process_name) {
  ...
  Satan.executeRemote('reloadProcessName', process_name, function(err) {
    ...
    speedList();
    console.log(cst.PREFIX_MSG + 'Processes name %s reloaded', process_name);
  });
};
...
CLI.streamLogs = function(id) {
  ...
  Satan.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    list.forEach(function(l) {
      tdb[l.pm2_env.pm_exec_path + '-' + l.pm2_env.pm_id] = l;
    });
