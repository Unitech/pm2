...
CLI.startup = function(platform, opts, cb) {
  ...
  if (platform == 'systemd') {
    cmd = 'su -l ' + user + ' -c "' + cmdAsUser + '" && su root -c "' + cmd + '"';
  }else if (platform == 'freebsd') {
    cmd = 'su root -c "' + cmd + '"';
  }else if (platform != 'darwin') {
    cmd = 'su -c "' + cmd + '"';
...
function mergeEnvironmentVariables(app, envName, deployConf) {
  ...
  // Update configuration variables (timestamp format, max-memory-restart..)
  delete new_conf.env;
  util._extend(app.env, new_conf);
