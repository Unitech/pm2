...
CLI.startFile = function(script) {
  ...
  if (appConf['name'] == 'run.sh' || appConf['name'] == 'index'){
    dirs = p.dirname(script).split(p.sep);
    if(dirs[dirs.length-1] == 'bin' && dirs.length >= 2){
      appConf['name'] = dirs[dirs.length-2];
    }
    else{
      appConf['name'] = dirs[dirs.length-1];  
    }
  }
  ...
  if (commander.executeCommand || appConf['exec_interpreter'] != 'node')
