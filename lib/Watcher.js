...
module.exports = function ClusterMode(God) {
  ...
  God.watch.enable = function(pm2_env) {
    ...
    if (pm2_env.watch && util.isArray(pm2_env.watch)) {
      watch = pm2_env.watch.map(function(dir) {
          return p.resolve(process.env.PWD, dir);
      });
    } else if (pm2_env.watch && (typeof pm2_env.watch == 'string' || pm2_env.watch instanceof String)) {
        watch = p.resolve(process.env.PWD, pm2_env.watch);
    } else {
