...
module.exports = function(God) {
  ...
  God.registerCron = function(pm2_env) {
    if (!pm2_env ||
        ...
        God.CronJobs.has(pm2_env.pm_id))
      return;
    ...
    job.start();
    God.CronJobs.set(`cron-${pm2_env.pm_id}`, job);
  }
  ...
  God.deleteCron = function(id) {
    if (typeof(id) !== 'undefined' && God.CronJobs.has(`cron-${id}`) === false)
      return;
    console.log('[PM2] Deregistering a cron job on:', id);
    var job = God.CronJobs.get(`cron-${id}`);
    job.stop();
    God.CronJobs.delete(`cron-${id}`);
  };
