  ...
  for (var id in God.clusters_db) {
    if (God.clusters_db[id].pm2_env.exec_mode == 'cluster_mode') {
      ...
      (function (oldWorker, currentWorkerIndex) {
        var tmpOldWorker = oldWorker;
        var newProc = JSON.parse(JSON.stringify(oldWorker.pm2_env));

        newProc.pm_id = God.getNewId();
        newProc.restart_time += 1;

        // Create new copy of process
        executeApp(newProc, function(err, newWorker) {
          ...
          newWorker.once('listening', function(){
            ...
            oldWorker.once('disconnect', function(){
              ...
              God.deleteProcessId(oldWorker.pm2_env.pm_id, function(){
