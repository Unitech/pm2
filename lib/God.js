...
function handleExit(clu, exit_code) {
  ...
  if ((Date.now() - proc.pm2_env.created_at) < (min_uptime * max_restarts)) {
    ...
    if (proc.pm2_env.unstable_restarts >= max_restarts) {
      ...
      console.log('Script %s had too many unstable restarts (%d). Stopped. %j',
                  ...
                  proc.pm2_env.status);
      God.bus.emit('process:exit:overlimit', { process : Common.serialize(proc) });
  ...
  God.bus.emit('process:exit', { process : Common.serialize(proc) });
...
God.prepare = function prepare(env, cb) {
  ...
  if (env.instances) {
    ...
    (function ex(i) {
      ...
      return God.executeApp(Common.serialize(env), function(err, clu) { // deep copy
