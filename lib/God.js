...
function executeApp(env, cb) {
  if (env['pm_id'] === undefined) {
    ...
    // Separate all logs
    env.pm_out_log_path = env.pm_out_log_path.replace('.log', '-' + env['pm_id'] + '.log');
    env.pm_err_log_path = env.pm_err_log_path.replace('.log', '-' + env['pm_id'] + '.log');
  }
...
God.reloadProcessId = function(id, cb) {
  ...
  God.clusters_db['todelete' + id] = God.clusters_db[id];
  delete God.clusters_db[id];

  var old_worker = God.clusters_db['todelete' + id];
  // Deep copy
  ...
  executeApp(new_env, function(err, new_worker) {
    ...
    new_worker.once('listening', function(){
      ...
      old_worker.once('disconnect', function(){
        ...
        God.deleteProcessId('todelete' + id, cb);
      });
...
God.reload = function(env, cb) {
  ...
  (function ex(processes, i) {
    if (i <= -1) return cb(null, {process_restarted : l});
    if (processes[i].state == cst.STOPPED_STATUS ||
        processes[i].pm2_env.exec_mode != 'cluster_mode') return ex(processes, i - 1);
    return God.reloadProcessId(processes[i].pm2_env.pm_id, function() {
      ex(processes, i - 1);
    });
  })(pros, l - 1);
};
