...
var fs            = require('fs');
var vizion        = require('../modules/vizion');
...
// silent: workers get piped stdout/stderr (read in ClusterMode.js) instead
// of inheriting the daemon's fds, which point to pm2.log
cluster.setupMaster({
  windowsHide: true,
  silent: true,
  exec : path.resolve(path.dirname(module.filename),
    cst.IS_BUN == true ? 'ProcessContainerBun.js' : 'ProcessContainer.js')
});
...
God.writeExitSeparator = function(pm2_env, code, signal) {
  try {
    ...
    if (pm2_env.pm_out_log_path)
      fs.appendFileSync(pm2_env.pm_out_log_path, exit_sep)
    if (pm2_env.pm_err_log_path)
      fs.appendFileSync(pm2_env.pm_err_log_path, exit_sep)
    if (pm2_env.pm_log_path)
      fs.appendFileSync(pm2_env.pm_log_path, exit_sep)
  } catch(e) {
  }
}
...
God.executeApp = function executeApp(env, cb) {
  ...
  // Flatten env vars into pm2_env without letting a shell variable named like a
  // PM2 option (name, namespace, cron_restart, instances...) override the config
  Utility.extendEnv(env_copy, env_copy.env);
  ...
  var readyCb = function ready(proc) {
    // Idempotent: exit-before-online, ready timeout and the regular online
    // path can race — only the first conclusion may answer the caller
    if (cb_called) return false;
    cb_called = true
  ...
  if (env_copy.exec_mode === 'cluster_mode') {
    ...
    God.nodeApp(env_copy, function nodeApp(err, clu) {
      ...
      clu.once('exit', function cluExit(code, signal) {
        ...
        // Worker died before emitting 'online' (bad execArgv, OOM at boot…):
        // conclude executeApp's callback so callers (start/restart RPC) are
    ...
    God.forkMode(env_copy, function forkMode(err, clu) {
      ...
      clu.once('exit', function cluClose(code, signal) {
        ...
        // Process died before the wait_ready conclusion: answer the caller
        // now instead of waiting for the ready timeout
        if (cb_called == false)
          readyCb(clu);
...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  ...
  // A late exit event can belong to a previous generation of this pm_id whose
  // slot has already been taken over by a replacement (seen on Windows with
  // shutdown_with_message, #6142): it must not affect the current occupant
  if (proc !== clu) {
    console.log(`App [${clu.pm2_env.name}:${clu.pm2_env.pm_id}] stale exit event ignored (pid ${clu.process.pid})`);
    return false;
  }
