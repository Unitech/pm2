...
function execute(env, cb) {
  ...
  if (env.separate_log_file) {
    env.pm_out_log_path = env.pm_out_log_path.replace('.log', '-' + id + ".log");
    env.fileOutput = env.pm_out_log_path;
    env.pm_err_log_path = env.pm_err_log_path.replace('.log', '-' + id + ".log");
    env.fileError = env.pm_err_log_path;
  }
