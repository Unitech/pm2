...
God.startDocker = function startDocker(cmd, cb) {
  var container = docker.getContainer(cmd);
  ...
  container.inspect(function(err, data) {
    ...
    container.start(function(err, data) {
      if (err)
        return cb(new Error("Could not start the container for some reason"));
      ...
      container.inspect(function(err, data) {
        ...
        env.pm2_env = JSON.parse(JSON.stringify(env));

        God.clusters_db[env.pm_id] = env;
      });

      console.log('Container ' + cmd + ' started');

      container.attach({stream: true, stdout: true, stderr: true}, function (err, stream) {
        stream.on('data', function(data) {
          God.bus.emit('log:out', {
            process : {
              pm_id      : env.pm2_env.pm_id,
              name       : env.pm2_env.name,
              rev        : null
            },
            at  : Utility.getDate(),
            data : data.toString()
      ...
      container.wait(function() {
        God.clusters_db[env.pm_id]['pm2_env']['status'] = cst.STOPPED_STATUS;
        console.log('Container ' + cmd + ' stopped');
      });
      cb(null, 'Container ' + cmd + ' started');
    });
  });
};

God.restartDocker = function restartDocker(cmd, cb) {
};

God.stopDocker = function stopDocker(cmd, cb) {
  var proc = null;

  function getProcessIdByName(name, cb) {
    var ret = [];
    God.getFormatedProcesses().forEach(function(elem) {
      if (elem.name === name)
        ret.push(elem);
    });
    return cb(null, ret);
  ...
  function checkId(id) {
    if (!isNaN(parseInt(id))) {
      if (God.clusters_db[id] === undefined)
        return cb(new Error('Process id ' + cmd + ' not found'));
      if (God.clusters_db[id].pm2_env.exec_mode !== 'docker_mode')
        return cb(new Error('Process id ' + cmd + ' is not a container'));
      proc = God.clusters_db[id];
    }
  }

  function stopContainer(proc) {
    var name = proc.name;
    ...
    container.stop(function(err, data) {
      if (err)
        return cb(new Error(err));
      proc.pm2_env.status = cst.STOPPED_STATUS;
      return cb(null, 'Container ' + name + ' stopped');
    });
  };
  ...
  if (proc === null) {
    getProcessIdByName(cmd, function(err, processes) {
      if (processes.length == 0) {
        return cb(new Error('Process id ' + cmd + ' not found'));
      }
      else {
        checkId(processes[0].pm_id);
        if (proc)
          stopContainer(proc);
        else
          return cb(new Error('Process id ' + cmd + ' is not a container'));
