...
function handleExit(clu, exit_code) {
  ...
  /**
   * if the process keep executing over 15 seconds
   * this may not be an unstable restart
   * and should not count in infinite reloop
   * so reset unstable_restarts to 0
   */
  if ((Date.now() - clu.pm2_env.pm_uptime) > 15000) {
   ...
   */
  // if the process has an uptime less than a second
...
God.restartProcessName = function(name, cb) {
  ...
  async.eachLimit(processes, cst.CONCURRENT_ACTIONS, function(proc, next) {
    if (p.basename(proc.pm2_env.pm_exec_path) == name || proc.pm2_env.name == name) {
      if (proc.pm2_env.status == cst.ONLINE_STATUS) {
        process.kill(proc.pid);
        return setTimeout(next, 500);
      }
      else if (proc.pm2_env.status == cst.STOPPED_STATUS) {
        God.startProcessId(proc.pm2_env.pm_id, function() {
          return setTimeout(next, 500);
        });
        return false;
      }
