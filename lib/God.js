...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  ...
  //var check_repeating_reset = typeof(proc.pm2_env.check_unstable_after_online) !== 'undefined' ? proc.pm2_env.check_unstable_after_online : false;

  console.log('-----------------------------');
  console.log(proc.pm2_env.check_unstable_after_online);
  console.log(proc.pm2_env.max_restarts);
  console.log(proc.pm2_env.repeating_reset);
  console.log('-----------------------------');

  //Only if process restart in repiting restart count ++ if process not run minimal uptime
  if ((Date.now() - proc.pm2_env.pm_uptime) < min_uptime && proc.pm2_env.check_repeating_reset == true) {
    // Increment unstable restart if check repeating
    proc.pm2_env.repeating_reset += 1;
  } else {
    proc.pm2_env.repeating_reset = 0;
  }
  ...
  if (proc.pm2_env.unstable_restarts >= max_restarts || proc.pm2_env.repeating_reset>= max_restarts) {
