        ...
        executeApp(oldWorker.pm2_env, function(err, newWorker) {
          newWorker.once('listening', function(){
            ...
            oldWorker.once('disconnect', function(){
              ...
              God.stopProcessId(oldWorker.pm2_env.pm_id, oldWorker.process.pid, function(){
...
God.stopProcessId = function(id, pid, cb) {
  if (!cb) { cb = pid; pid = null; }
  if (!(id in God.clusters_db))
    return cb(new Error({msg : "PM ID unknown"}), {});
  ...
  var _pid = pid || God.clusters_db[id].process.pid;
  if (_pid != 0) {
    try {
