...
God.injectVariables = function injectVariables (env, cb) {
  ...
  if (env.increment_var) {
    ...
    var defaultEnv = env.env[env.increment_var] || 0;

    // Check wether the environment variable is a number.
    // If not, parse it to an integer so that the value can be incremented instead of append.
    var parsedEnv = defaultEnv;
    if(typeof parsedEnv === 'string') {
      parsedEnv = parseInt(parsedEnv);

      // If somehow parsing fails, reset the variable into original state.
      if(isNaN(parsedEnv)){
        parsedEnv = defaultEnv
      }
    }
    
    env[env.increment_var] = typeof lastIncrement === 'undefined' ? parsedEnv : lastIncrement + 1;
