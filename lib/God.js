...
God.prepare = function prepare (env, cb) {
  ...
  if (typeof env.instances === 'undefined') {
    ...
    return God.executeApp(env, function (err, clu) {
      ...
      return realizeQueue(clu, cb, true);
    });
  }
  ...
  async.timesLimit(env.instances, 1, function (n, next) {
    ...
    God.injectVariables(env, function inject (err, _env) {
      ...
      return God.executeApp(Utility.clone(_env), function (err, clu) {
        ...
        realizeQueue(clu, next, false);
      });
    });
  }, cb);

  // This function just for put in order code in callbacks
  // It function realize principe: next process start launch,
  // when prev process send signal 'ready' or 'pm2:started'
  
  function realizeQueue(clu, cb, isArrayReturn) {
    if (clu && clu.pm2_env && clu.pm2_env.priority) {
      God.bus.on('process:msg', function waitOnline(rawMsg) {
        if (rawMsg.raw &&
          ((rawMsg.raw.indexOf('pm2') !== -1 &&
            rawMsg.raw.indexOf('started') !== -1) ||
            rawMsg.raw === 'ready') &&
            rawMsg.process.pm_id === clu.pm2_env.pm_id) {

              God.bus.removeListener('process:msg', waitOnline);
              return cb(null, (isArrayReturn ? [Utility.clone(clu)] : Utility.clone(clu)));
        }
      });
    } else {
      return cb(null, (isArrayReturn ? [Utility.clone(clu)] : Utility.clone(clu)));
