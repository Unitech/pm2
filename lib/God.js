...
God.executeApp = function executeApp(env, cb) {
  ...
  if (env_copy['pm_id'] === undefined) {
    ...
    env_copy['unstable_restarts_in_row'] = 0;
...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  ...
  if ((Date.now() - proc.pm2_env.pm_uptime) < min_uptime) {
    // During startup
    if ((Date.now() - proc.pm2_env.created_at) < ((min_uptime + restart_delay) * max_restarts)) {
      // Increment unstable restart
      proc.pm2_env.unstable_restarts += 1;
    }
    proc.pm2_env.unstable_restarts_in_row += 1;
  } else {
    proc.pm2_env.unstable_restarts_in_row = 0;
  }


  if (proc.pm2_env.unstable_restarts >= max_restarts || proc.pm2_env.unstable_restarts_in_row >= max_restarts) {
