...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  ...
  if ((Date.now() - proc.pm2_env.created_at) < (min_uptime * cst.MAX_ERRORED_RESTARTS)) {
  ...
  if (proc.pm2_env.unstable_restarts >= cst.MAX_ERRORED_RESTARTS) {
  ...
  if (proc.pm2_env.max_restarts && proc.pm2_env.restart_time > proc.pm2_env.max_restarts) {
    proc.process.pid = 0;
    proc.pm2_env.unstable_restarts = 0;
    proc.pm2_env.created_at = null;
    overlimit = true;
