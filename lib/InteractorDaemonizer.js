...
var debug = require('debug')('Interactor:daemonizer');
...
InteractorDaemonizer.launchOrAttach = function(secret_key, machine_name, cb) {
  InteractorDaemonizer.ping(function(online) {
    if (online) {
      return cb(false);
    }
    else
      InteractorDaemonizer.daemonize(secret_key, machine_name, function() {
        return cb(true);
      });
    return false;
  });
...
InteractorDaemonizer.daemonize = function(secret_key, machine_name, cb) {
  ...
  var child = require('child_process').fork(InteractorJS, [], {
    ...
    console.log('Interactor daemonized');
  });
