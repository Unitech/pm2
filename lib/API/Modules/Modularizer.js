...
function installLangModule(module_name, cb) {
  ...
  var install_instance = spawn(cst.IS_WINDOWS ? 'npm.cmd' : npm_bin_path, ['install', module_name, '--loglevel=error'], {
    stdio : 'inherit',
    env: process.env,
		shell : true,
...
function getNpmBin() {
  var npm_bin_path = 'npm';

  if (fs.existsSync(path.join(__dirname, '../../../node_modules/npm/bin/npm-cli.js'))) {
    npm_bin_path = path.join(__dirname, '../../../node_modules/npm/bin/npm-cli.js');
  }
