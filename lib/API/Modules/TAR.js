...
function install(CLI, module_filepath, opts, cb) {
  Common.log(`${cst.PREFIX_MSG_MOD} Unpacking local tarball ${module_filepath}`)
  ...
  getModuleName(module_filepath, function(err, module_name) {
    ...
    install_instance.on('close', function(code) {
      if (code == 0)
        return run(CLI, install_path, module_name, code, cb)
...
function run(PM2, target_path, module_name, code, cb) {
