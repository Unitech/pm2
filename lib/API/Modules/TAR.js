...
function deleteModulePath(module_name) {
  ...
  require('shelljs').exec(`rm -r ${path.join(cst.DEFAULT_MODULE_PATH, module_name)}`, { silent: true })
}
...
function getTarCmd(target_fullpath, base_folder, module_folder_name, cb) {
  var isBSDTar = false

  var tar_version = exec('tar --version', (err, sto, ste) => {
    if (err) {
      console.log(sto.toString().trim())
      console.log(ste.toString().trim())
    }

    isBSDTar = sto.indexOf('bsdtar') !== -1
  })

  tar_version.on('close', function (code) {
    var transform_arg = isBSDTar
      ? `-s /${module_folder_name}/module/`
      : `--transform 's,${module_folder_name},module,'`
    var tar_cmd = `tar zcf ${target_fullpath} -C ${base_folder} ${transform_arg} ${module_folder_name}`

    cb(code == 0 ? null : code, tar_cmd)
  })
}
...
function package(module_path, target_path, cb) {
  ...
  getTarCmd(target_fullpath, base_folder, module_folder_name, (err, cmd) => {
    if (err) {
      Common.errMod('Can\'t run tar command, exiting')
      process.exit(1)
    }
...
function publish(PM2, folder, cb) {
  ...
  package(current_path, target_path, (err, res) => {
    ...
    var uri = `${PM2.user_conf.registry}/api/v1/modules`
