...
function packager(module_path, target_path, cb) {
  ...
  var tar = spawn('tar', ['zcf', target_fullpath, '-C', base_folder, '--transform', `s,${module_folder_name},module,`, module_folder_name], {
    stdio: 'inherit'
  })
...
function publish(PM2, folder, cb) {
  ...
  packager(current_path, target_path, (err, res) => {
    ...
    var fileBuffer = fs.readFileSync(res.path)
    formData.append('module_data', new Blob([fileBuffer], { type: 'content/gzip' }), path.basename(res.path))
    formData.append('id', pkg.name)
...
function needPrefix(conf) {
  if ((conf.apps && conf.apps.length > 1) ||
      ...
      (conf.apps && conf.apps.length == 1 && conf.name != conf.apps[0].name))
    return true
  return false
}
