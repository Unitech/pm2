...
function switchDockerFile(docker_filepath, main_file, mode) {
  return new Promise(function(resolve, reject) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (line.indexOf('ENV NODE_ENV=') > -1) {
        lines[i] = 'ENV NODE_ENV=' + (mode == 'distribution' ? 'production' : mode);
      }

      if (line.indexOf('CMD [') > -1) {
        if (mode == 'development')
          lines[i] = 'CMD ["rundev", "start", "--auto-exit", "' + main_file + '"]';
        if (mode == 'distribution') {
          delete lines[i];
          lines.splice(i, 0, 'COPY . /var/app');
          lines.splice(i + 1, 0, 'CMD ["pm2-docker", "start", "--auto-exit", "' + main_file + '", "--env", "production"]');
          break;
        }
      }
    };
...
function generateDockerfile(docker_filepath, main_file, mode) {
  return new Promise(function(resolve, reject) {
    ...
    if (mode == 'test') {
      template += 'ENV NODE_ENV=test\n';
      template += 'CMD ["npm", "test"]';
      CMD = 'npm test';
    }
    else if (mode == 'distribution') {
      template += 'ENV NODE_ENV=production\n';
      template += 'COPY . /var/app/\n';
      CMD = 'CMD ["pm2-docker", "start", "--auto-exit", "' + main_file + '", "--env", "production"]';
      template += CMD;
    }
    else {
      template += 'ENV NODE_ENV=development\n';
      //CMD = 'CMD ["rundev", "start", "--auto-exit", "' + main_file + '", "--env", "development"]';
      CMD = 'CMD ["rundev", "start", "--auto-exit", "' + main_file + '"]';
      template += CMD;
    }
