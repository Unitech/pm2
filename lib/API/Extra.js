...
module.exports = function(CLI) {
  ...
  CLI.prototype.dashboard = function(opts, cb) {
    ...
    var lines = Dashboard.parseLines(opts.lines);
    // max time spent reading log files before showing the dashboard
    var PRELOAD_TIMEOUT = 2000;

    Dashboard.init({ lines: lines });

    // Live log events received while the history is being read from the
    // log files are buffered, then replayed after the history so the
    // stream stays chronological and nothing is lost
    var preloading = true;
    var pending = [];
    ...
    this.Client.launchBus(function (err, bus) {
      ...
      bus.on('log:*', function(type, data) {
        if (preloading)
          return pending.push([type, data]);
        Dashboard.log(type, data)
      })
    });

    // Preload the last `lines` lines (out + err merged) of every process
    function preloadHistory(list, done) {
      var finished = false;
      var finish = function() {
        if (finished) return;
        finished = true;
        preloading = false;
        // replay skips lines already loaded from the files
        pending.forEach(function(ev) { Dashboard.replay(ev[0], ev[1]); });
        pending = [];
        done();
      };
      // never block the dashboard on slow/huge log files
      var timer = setTimeout(finish, PRELOAD_TIMEOUT);

      forEachLimit(list, 4, function(proc, next) {
        var env = proc.pm2_env || {};
        var identity = { pm_id: proc.pm_id, name: env.name };

        Log.getLastLines(env.pm_out_log_path, lines, function(out_lines) {
          Log.getLastLines(env.pm_err_log_path, lines, function(err_lines) {
            // too late (timeout hit, live lines already flowing): history
            // must not be appended after live lines
            if (!finished)
              Dashboard.preload(identity, out_lines, err_lines);
            next();
          });
        });
      }, function() {
