...
module.exports = function(CLI) {
  ...
  CLI.prototype.pullCommitId = function(process_name, commit_id, cb) {
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      ...
      var proc = processes[0];
      if (proc.pm2_env.versioning) {
        require('../../modules/vizion').isUpToDate({folder: proc.pm2_env.versioning.repo_path}, function(err, meta) {
          if (err !== null)
            return cb ? cb({msg:err}) : that.exitCli(cst.ERROR_EXIT);
          require('../../modules/vizion').revertTo(
            {revision: commit_id,
             folder: proc.pm2_env.versioning.repo_path},
            function(err2, meta2) {
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      ...
      require('../../modules/vizion').prev({
        ...
        getPostUpdateCmds(proc.pm2_env.versioning.repo_path, process_name, function (command_list) {
          execCommands(proc.pm2_env.versioning.repo_path, command_list, function(err, res) {
            if (err !== null) {
              require('../../modules/vizion').next({folder: proc.pm2_env.versioning.repo_path}, function(err2, meta2) {
                printError(err);
                return cb ? cb({msg: meta.output + err}) : that.exitCli(cst.ERROR_EXIT);
              });
              return false;
  ...
  CLI.prototype.forward = function(process_name, cb) {
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      ...
      process_name = proc.name;
      if (proc.pm2_env.versioning) {
        require('../../modules/vizion').next({folder: proc.pm2_env.versioning.repo_path}, function(err, meta) {
          if (err !== null)
            return cb ? cb({msg:err}) : that.exitCli(cst.ERROR_EXIT);
          if (meta.success === true) {
            getPostUpdateCmds(proc.pm2_env.versioning.repo_path, process_name, function (command_list) {
              execCommands(proc.pm2_env.versioning.repo_path, command_list, function(err, res) {
                if (err !== null)
                {
                  require('../../modules/vizion').prev({folder: proc.pm2_env.versioning.repo_path}, function(err2, meta2) {
                    printError(err);
                    return cb ? cb({msg:meta.output + err}) : that.exitCli(cst.ERROR_EXIT);
                  });
                }
