...
module.exports = function(CLI) {
  ...
  CLI.prototype.processesAreAlreadyMonitored = function(cb) {
  ...
  CLI.prototype.minimumSetup = function (opts, cb) {
    ...
    this.processesAreAlreadyMonitored((already_monitored) => {
      if (already_monitored) {
        console.log(chalk.cyan('[PM2.IO]') + ' PLUS bundle already installed');
        return cb()
      }
          ...
          if (buckets.length > 1) {
            ...
            (function retryInsertion() {
              promptly.prompt('Type the bucket you want to link to: ', function(err, bucket_name) {
                ...
                return link(target_bucket, cb);
              });
            })();
          }
          else {
            var target_bucket = buckets[0];
            link(target_bucket, cb)
          }
        }).catch(err => {
  ...
  CLI.prototype.registerToKM = function(opts) {
    printMotd();
    ...
    function finalize(bucket) {
      self.minimumSetup(opts, () => {
        ...
        console.log(chalk.green('[PM2.IO]') + ' Access to the remote dashboard: https://app.pm2.io/#/r/' + bucket.public_key);
    ...
    promptly.confirm(chalk.bold('Do you have a pm2.io account? (y/n)'), (err, answer) => {
      ...
      // Login
      if (answer == true) {
        return CLI.prototype.loginToKM((err, bucket) => {
  ...
  function link(target_bucket, cb) {
