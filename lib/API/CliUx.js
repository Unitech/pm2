  ...
  list.forEach(function(l) {
    ...
    if (l.pm2_env.pmx_module == true) {
      ...
      if (!stacked) {
        ...
        obj[key].push(chalk.bold(l.pm2_env.axm_options.module_version || 'N/A'), pid);
      }
      ...
      if (!stacked) {
        if (l.pm2_env.uid && typeof(l.pm2_env.uid) == 'number') {
          ...
          l.pm2_env.uid = Object.keys(users).reduce((acc, username)  => {
            if (users[username].userId == l.pm2_env.uid)
              acc = users[username].name
          })
        }
        obj[key].push(chalk.bold(l.pm2_env.uid || l.pm2_env.username));
      }
