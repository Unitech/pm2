...
UX.dispAsTable = function(list, interact_infos) {
  ...
  var app_head = stacked ? ['Name', 'id', 'mode', 'status', 'cpu', 'mem'] : 
  ...
  var app_table = new Table({
    head : app_head,
    colAligns : new Array(app_head).fill('left') || ['left'],
    style : {'padding-left' : 1, head : ['cyan', 'bold'], compact : true}
  });

  var module_table = new Table({
    head : mod_head,
    colAligns : new Array(mod_head).fill('left') || ['left'],
    style : {'padding-left' : 1, head : ['cyan', 'bold'],  compact : true}
  });
  ...
  list.forEach(function(l) {
    ...
    if (l.pm2_env.pmx_module == true) {
      ...
      obj[key] = [ l.pm2_env.pm_id, mode == 'fork_mode' ? chalk.inverse.bold('fork') : chalk.blue.bold('cluster') ];
      if (!stacked)
        obj[key].push(l.pid);
      obj[key].push(colorStatus(status));
      if (!stacked) 
