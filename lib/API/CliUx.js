...
UX.dispAsTable = function(list, sys_infos) {
  ...
  var name_col_size = 10

  if (list && list.length > 0)
    name_col_size = (list.reduce((p, c) => (p.name.length > c.name.length) ? p : c)).name.length + 4
  ...
  if (sys_infos && sys_infos.cpu && sys_infos.cpu.usage) {
    ...
    if (sys_infos.mem) {
      ...
      sys_summary_line += ` | ${chalk.bold('mem free')}: ${colorizedMetric(perc_mem_usage, 30, 10, '%')} `
    }
