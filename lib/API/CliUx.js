...
UX.dispAsTable = function(list, sys_infos) {
  ...
  var app_table = new Table({
    head : app_head,
    colWidths: [4,20,9,7,7,8,6,10,10,10,10,10],
    colAligns : ['left'],
    style : {'padding-left' : 1, head : ['cyan', 'bold'], compact : true}
  });
  ...
  list.forEach(function(l) {
    ...
    if (l.pm2_env.pmx_module == true) {
      ...
      obj[key].push(l.pm_id)

      // Module version + PID
      if (!stacked) {
        var pid = l.pm2_env.axm_options.pid ? l.pm2_env.axm_options.pid : l.pid
        obj[key].push(chalk.bold(l.pm2_env.version || 'N/A'), pid);
      }
  ...
  if (sys_infos && sys_infos.containers && sys_infos.containers.length > 0) {
    ...
    var docker_head = stacked_docker ? ['id', 'name', 'state', 'cpu', 'mem'] :
        ['id', 'name', 'image', 'tag', 'status', 'cpu', 'mem', 'net IO ⇵', 'fs IO ⇵'];

    var docker_table = new Table({
      colWidths: [4,25,25,10,10, 10,10,10,10],
      head : docker_head,
      colAligns : ['left'],
      style : {'padding-left' : 1, head : ['cyan', 'bold'],  compact : true}
    });
    ...
    sys_infos.containers.forEach((c) => {
      ...
      var image = img[0]
      var tag = img.length > 1 ? img[1] : ''

      if (stacked_docker)
        docker_table.push([id, c.name, state, `${cpu}%`, `${mem}mb`])
      else {
        docker_table.push([
          id,
          c.name,
  ...
  if (sys_infos && sys_infos.processes) {
    ...
    var sys_proc_head = ['id', 'name', 'cmd', 'pid', 'cpu', 'mem', 'uid']

    var sys_proc_table = new Table({
      colWidths: [4,25,30,10,10,10,8],
      head : sys_proc_head,
      colAligns : ['left'],
      style : {'padding-left' : 1, head : ['cyan', 'bold'],  compact : true}
    });
    ...
    if (sys_infos.processes.cpu_sorted || sys_infos.processes.mem_sorted) {
      sys_infos.processes.cpu_sorted = sys_infos.processes.cpu_sorted.filter((proc) => {
        return proc.cpu > CPU_MIN_SHOW && proc.name.includes('node') === false
      })
      sys_infos.processes.cpu_sorted.forEach(proc => {
        ...
        var cmd = proc.cmd
        if (proc.cmd.length > 30)
          cmd = '…' + proc.cmd.slice(proc.cmd.length - 27, proc.cmd.length)
        sys_proc_table.push([chalk.bold.cyan(proc_id++), proc.name, cmd, proc.pid, cpu, mem, proc.uid])
      })

      sys_infos.processes.mem_sorted = sys_infos.processes.mem_sorted.filter((proc) => {
        return proc.memory > MEM_MIN_SHOW && proc.name.includes('node') === false
      })
  ...
  if (sys_infos) {
    ...
    if (sys_infos.cpu.temperature)
      sys_summary_line += ` ${colorizedMetric(sys_infos.cpu.temperature, 50, 70, 'º')}`
    if (sys_infos.mem) {
      var perc_mem_usage = (((sys_infos.mem.total - sys_infos.mem.free) / sys_infos.mem.total) * 100).toFixed(1)
  ...
  if (module_table.length > 0 && module_table.length == 1) {
