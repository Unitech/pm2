...
// Recomputed in listModulesAndAppsManaged() against the real table width;
// this initial value is the fallback for the other display functions.
let CONDENSED_MODE = (process.stdout.columns || 300) < 134
...
// Truncate before colorizing: cli-tableau's truncate() miscounts ANSI bytes, cutting styled strings too short and leaking style into the next column.
function fitColumn(str, colWidth) {
  // cli-tableau content width = colWidth - padding-left(1) - padding-right(1)
...
function listModulesAndAppsManaged(list, commander) {
  // Cap the (otherwise unbounded) name column so a single long process
  // name can't push the table past the terminal width.
  var NAME_COL_MAX = 40
  var name_col_size = 11

  if (list && list.length > 0)
    name_col_size = Math.min((list.reduce((p, c) => (p.name.length > c.name.length) ? p : c)).name.length + 5, NAME_COL_MAX)
  ...
  list.forEach(function(l) {
    ...
    if (l.pm2_env.pmx_module == true) {
      ...
      if (!CONDENSED_MODE) {
        ...
        obj[key].push(chalk.bold(fitColumn(l.pm2_env.uid || l.pm2_env.username, mod_head.user)))
      }
      ...
      // PM2 ID
      obj[key].push(fitColumn(l.pm2_env.name, app_head.name))
      ...
      if (!CONDENSED_MODE) {
        ...
        obj[key].push(chalk.bold(fitColumn(l.pm2_env.uid || l.pm2_env.username, app_head.user)))
      }
...
function miniMonitBar(sys_infos) {
  ...
  // Throughput in mb/s -> "12.3mb/s", or "480kb/s" under 1mb/s (thresholds are in mb/s)
  const rate = (mb, warn, alert) => {
    mb = Number(mb) || 0
    if (mb > 0 && mb < 1)
      return UxHelpers.colorizedMetric(Math.round(mb * 1024), warn * 1024, alert * 1024, 'kb/s')
    return UxHelpers.colorizedMetric(mb == 0 ? 0 : Number(mb.toFixed(1)), warn, alert, 'mb/s')
  }
  // Keep long mount paths readable: "…/last/two/segments"
  const shortMount = (m, max = 24) => {
    if (m.length <= max) return m
    let parts = m.split('/').filter(Boolean)
  ...
  let cpu_seg = `${chalk.bold('cpu')}: ${UxHelpers.colorizedMetric(cpu.value, 40, 70, '%')}`
  let temp = sys_metrics['CPU Temperature'] ? sys_metrics['CPU Temperature'].value : null
  if (temp && temp != '-1')
    cpu_seg += ` ${UxHelpers.colorizedMetric(temp, 50, 70, 'º')}`
  segs.push(cpu_seg)

  let ram_usage = sys_metrics['RAM Usage'] ? sys_metrics['RAM Usage'].value : null
  if (ram_usage === null) {
    let mem_total = sys_metrics['RAM Total'] ? sys_metrics['RAM Total'].value : null
    let mem_available = sys_metrics['RAM Available'] ? sys_metrics['RAM Available'].value : null
    if (mem_total)
      ram_usage = (100 - ((mem_available / mem_total) * 100)).toFixed(1)
  }
  if (ram_usage !== null && ram_usage !== undefined)
    segs.push(`${chalk.bold('ram usage')}: ${UxHelpers.colorizedMetric(ram_usage, 70, 90, '%')}`)

  // GPU memory + temperature (only when a GPU reports)
  if (sys_metrics['graphics:mem:total'] && Number(sys_metrics['graphics:mem:total'].value) > 0) {
    let gpu_total = sys_metrics['graphics:mem:total'].value
    let gpu_used = sys_metrics['graphics:mem:used'] ? sys_metrics['graphics:mem:used'].value : 0
    let gpu_seg = `${chalk.bold('gpu')}: ${gpu_used}/${gpu_total}mb`
    let gpu_temp = sys_metrics['graphics:temp'] ? sys_metrics['graphics:temp'].value : null
    if (gpu_temp && gpu_temp != '-1')
      gpu_seg += ` ${UxHelpers.colorizedMetric(gpu_temp, 50, 70, 'º')}`
    segs.push(gpu_seg)
  }

  // Loopback, tunnels/VPN, container bridges and virtual NICs are noise in a one-line summary
  const VIRTUAL_IFACE = /^(lo\d*|utun\d*|tun\d*|tap\d*|wg\d*|ppp\d*|docker\d*|br-[0-9a-f]+|veth[0-9a-f]*|virbr\d*|vmnet\d*|vboxnet\d*|bridge\d*|awdl\d*|llw\d*|anpi\d*|ap\d*|gif\d*|stf\d*)$/
  ...
  interfaces.forEach(iface => {
    if (VIRTUAL_IFACE.test(iface)) return
    if (!sys_metrics[`net:rx_5:${iface}`]) return
    let rx = sys_metrics[`net:rx_5:${iface}`].value
    let tx = sys_metrics[`net:tx_5:${iface}`] ? sys_metrics[`net:tx_5:${iface}`].value : 0
    // Only show interfaces actually carrying traffic
    if (!(Number(rx) > 0 || Number(tx) > 0)) return
    let net_seg = `${chalk.bold(iface)}: ⇓ ${rate(rx, 10, 20)} ⇑ ${rate(tx, 10, 20)}`

    // Show errors/drops only when non-zero
    let rx_err = sys_metrics[`net:rx_errors_60:${iface}`] ? Number(sys_metrics[`net:rx_errors_60:${iface}`].value) : 0
    let tx_err = sys_metrics[`net:tx_errors_60:${iface}`] ? Number(sys_metrics[`net:tx_errors_60:${iface}`].value) : 0
    let rx_drop = sys_metrics[`net:rx_dropped_60:${iface}`] ? Number(sys_metrics[`net:rx_dropped_60:${iface}`].value) : 0
    let tx_drop = sys_metrics[`net:tx_dropped_60:${iface}`] ? Number(sys_metrics[`net:tx_dropped_60:${iface}`].value) : 0
    if (rx_err + tx_err > 0)
      net_seg += ` ${chalk.bold('err')} ${UxHelpers.colorizedMetric(rx_err + tx_err, 1, 10, '/min')}`
    if (rx_drop + tx_drop > 0)
      net_seg += ` ${chalk.bold('drop')} ${UxHelpers.colorizedMetric(rx_drop + tx_drop, 1, 10, '/min')}`
    segs.push(net_seg)
  })
  ...
  if (CONDENSED_MODE == false && sys_metrics['Disk Reads']) {
    ...
    let disk_seg = `${chalk.bold('disk')}: ⇓ ${rate(read, 10, 20)} ⇑ ${rate(write, 10, 20)}`
    ...
    disks.forEach(fs => {
      ...
      if (use > 60)
        disk_seg += ` ${chalk.gray(shortMount(fs))} ${UxHelpers.colorizedMetric(use, 80, 90, '%')}`
    })
    segs.push(disk_seg)
  }

  console.log(`${chalk.bold.cyan('host metrics')} | ${segs.join(' | ')}`)
}
