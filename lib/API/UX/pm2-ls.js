...
function checkIfProcessAreDumped(list) {
  try {
    ...
    if (diff.length > 0) {
      Common.warn(`Current process list is not synchronized with saved list. App ${chalk.bold(diff.join(' '))} differs. Type 'pm2 save' to synchronize or 'pm2 resurrect' for restore processes.`)
    }
    else if (apps_dumped.length != apps_running.length) {
      Common.warn(`Current process list is not synchronized with saved list. Type 'pm2 save' to synchronize or 'pm2 resurrect' for restore processes.`)
    }
  } catch(e) {
  }
}
...
function miniMonitBar(sys_infos) {
  ...
  if (ram_usage !== null && ram_usage !== undefined) {
    sys_summary_line += ` | ${chalk.bold('ram usage')}: ${UxHelpers.colorizedMetric(ram_usage, 70, 90, '%')} `
  }
  ...
  if (sys_metrics['graphics:mem:total'] && Number(sys_metrics['graphics:mem:total'].value) > 0) {
    ...
    sys_summary_line += `| ${chalk.bold('gpu')}: ${gpu_used}/${gpu_total}mb`
    let gpu_temp = sys_metrics['graphics:temp'] ? sys_metrics['graphics:temp'].value : null
    if (gpu_temp && gpu_temp != '-1')
      sys_summary_line += ` ${UxHelpers.colorizedMetric(gpu_temp, 50, 70, 'º')}`
    sys_summary_line += ` `
  }
  ...
  interfaces.forEach(iface => {
    ...
    sys_summary_line += `⇓ ${UxHelpers.colorizedMetric(rx, 10, 20, 'mb/s')} `
    sys_summary_line += `⇑ ${UxHelpers.colorizedMetric(tx, 10, 20, 'mb/s')} `
    ...
    if (rx_err + tx_err > 0)
      sys_summary_line += `${chalk.bold('err')} ${UxHelpers.colorizedMetric(rx_err + tx_err, 1, 10, '/min')} `
  ...
  console.log(sys_summary_line.trimEnd())
}
