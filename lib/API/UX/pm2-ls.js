...
function listModulesAndAppsManaged(list) {
  ...
  // allow customized column settings via env vars:
  // PM2_LS_APP_HEAD="id:5,name,mode,↺"
  // PM2_LS_MOD_HEAD="id:5,name:6,status"
  const heads = { app_head, mod_head }
  const custom_heads = {
    app_head: process.env.PM2_LS_APP_HEAD,
    mod_head: process.env.PM2_LS_MOD_HEAD
  }
  Object.keys(custom_heads).forEach(key => {
    const custom_head = custom_heads[key]
    if (!custom_head) return;
    let custom_cols = custom_head.split(/,/).reduce((acc, pair) => {
      const name_size = pair.trim().split(/:/)
      acc[name_size[0]] = parseInt(name_size[1], 10)
      return acc
    }, {})
    const current_cols = heads[key]
    Object.keys(current_cols).forEach(col_name => {
      if (custom_cols[col_name] === undefined || custom_cols[col_name] === 0) {
        delete current_cols[col_name]
      } else if (custom_cols[col_name] > 0) {
        current_cols[col_name] = custom_cols[col_name]
      }
    })
  })
