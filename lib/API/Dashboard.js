...
Dashboard.refresh = function(processes) {
  ...
  for (var i = 0; i < processes.length; i++) {
    ...
    this.metadataBox.setLine(1, 'Restarts              ' + proc.pm2_env.restart_time);
    this.metadataBox.setLine(2, 'Uptime                ' + ((proc.pm2_env.pm_uptime && proc.pm2_env.status == 'online') ? timeSince(proc.pm2_env.pm_uptime) : 0));
    this.metadataBox.setLine(3, 'Script path           ' + proc.pm2_env.pm_exec_path);
    this.metadataBox.setLine(4, 'Script args           ' + (proc.pm2_env.args ? (typeof proc.pm2_env.args == 'string' ? JSON.parse(proc.pm2_env.args.replace(/'/g, '"')):proc.pm2_env.args).join(' ') : 'N/A'));
    this.metadataBox.setLine(5, 'Interpreter           ' + proc.pm2_env.exec_interpreter);
    this.metadataBox.setLine(6, 'Interpreter args      ' + (proc.pm2_env.node_args.length != 0 ? proc.pm2_env.node_args : 'N/A'));
    this.metadataBox.setLine(7, 'Exec mode             ' + (proc.pm2_env.exec_mode == 'fork_mode' ? '{bold}fork{/}' : '{blue-fg}{bold}cluster{/}'));
    this.metadataBox.setLine(8, 'Node.js version       ' + proc.pm2_env.node_version);
    this.metadataBox.setLine(9, 'watch & reload        ' + (proc.pm2_env.watch ? '{green-fg}{bold}✔{/}' : '{red-fg}{bold}✘{/}'));
    this.metadataBox.setLine(10, 'Unstable restarts     ' + proc.pm2_env.unstable_restarts);
    this.metadataBox.setLine(10, 'Repeating restarts     ' + proc.pm2_env.repeating_reset);

    this.metadataBox.setLine(11, 'Comment               ' + ((proc.pm2_env.versioning) ? proc.pm2_env.versioning.comment : 'N/A'));
    this.metadataBox.setLine(12, 'Revision              ' + ((proc.pm2_env.versioning) ? proc.pm2_env.versioning.revision : 'N/A'));
    this.metadataBox.setLine(13, 'Branch                ' + ((proc.pm2_env.versioning) ? proc.pm2_env.versioning.branch : 'N/A'));
    this.metadataBox.setLine(14, 'Remote url            ' + ((proc.pm2_env.versioning) ? proc.pm2_env.versioning.url : 'N/A'));
    this.metadataBox.setLine(15, 'Last update           ' + ((proc.pm2_env.versioning) ? proc.pm2_env.versioning.update_time : 'N/A'));
    ...
    for(var monitor in proc.pm2_env.axm_monitor) {
      var prob = '{bold}' + monitor + '{/} {|} ' + proc.pm2_env.axm_monitor[monitor].value;

      if (this.metricsBox.getItem(j)) {
        this.metricsBox.setItem(j, prob);
      }
      else {
        this.metricsBox.pushItem(prob);
      }
      j++;
    }
