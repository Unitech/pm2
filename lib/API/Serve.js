...
http.createServer(function (request, response) {
  ...
  try {
    ...
    // never let a request handler exception kill the server
    console.error('[%s] Error while handling %s : %s', new Date(), request.url, e.message || e);
    if (!response.headersSent) response.writeHead(500, { 'Content-Type': 'text/html' });
...
function serveFile(uri, request, response) {
  var file;
  try {
    file = decodeURIComponent(new URL(uri || request.url, 'http://localhost').pathname);
  } catch (e) {
    // malformed percent-encoding (e.g. GET /%, /%zz) throws URIError
