...
module.exports = function(CLI) {
  ...
  function isNotRoot(platform, opts, cb) {
    if (opts.user) {
      ...
      console.log('      sudo su -c "env PATH=$PATH:' + path.dirname(process.execPath) + ' pm2 startup ' + platform + ' -u ' + opts.user + ' --hp ' + process.env.HOME + '"');
      return cb(new Error('You have to run this with elevated rights'));
    }
    return exec('whoami', function(err, stdout, stderr) {
      console.log(cst.PREFIX_MSG + 'You have to run this command as root. Execute the following command:');
      if (platform == 'launchd' || platform == 'darwin')
        ...
        console.log('      sudo su -c "env PATH=$PATH:' + path.dirname(process.execPath) + ' ' + require.main.filename + ' startup ' + platform + ' -u ' + stdout.trim() + ' --hp ' + process.env.HOME + '"');
      return cb(new Error('You have to run this with elevated rights'));
    });
  }
  ...
  CLI.prototype.uninstallStartup = function(platform, cb) {
    ...
    if (process.getuid() != 0) {
      if (platform == 'launchd')
        ...
        console.log('sudo su -c "env PATH=$PATH:' + path.dirname(process.execPath) + ' ' + require.main.filename + ' unstartup ' + platform + '"');
      return cb();
    }
