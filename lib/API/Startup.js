...
module.exports = function(CLI) {
  ...
  CLI.prototype.uninstallStartup = function(platform, opts, cb) {
    ...
    var service_name = 'pm2-' + (opts.user || process.env.USER);
    ...
    if (fs.existsSync('/etc/init.d/' + service_name + '-init.sh')) {
      platform = 'oldsystem';
    }
    ...
    case 'oldsystem':
      ...
      commands = [
        'update-rc.d ' + service_name + '-init.sh disable',
        'update-rc.d -f ' + service_name + '-init.sh remove',
        'rm /etc/init.d/' + service_name + '-init.sh'
      ];
      break;
    case 'openrc':
      commands = [
    ...
    case 'upstart':
      ...
      break;
    case 'launchd':
      var destination = path.join(process.env.HOME, 'Library/LaunchAgents/' + service_name.toUpperCase() + '.plist');
      commands = [
        'launchctl remove com.' + service_name.toUpperCase(),
        'rm ' + destination
      ];
    };
  ...
  CLI.prototype.startup = function(platform, opts, cb) {
    ...
    case 'launchd':
      ...
      destination = path.join(process.env.HOME, 'Library/LaunchAgents/' + service_name.toUpperCase() + '.plist');
      commands = [
