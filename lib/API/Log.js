...
Log.stream = function(Client, id, raw, timestamp, exclusive) {
  ...
  if (!raw)
    console.log(chalk.green(util.format.call(this, '[PM2] Streaming realtime logs for [%s] process%s', id, id === 'all' ? 'es' : '')));
  ...
  Client.launchBus(function(err, bus) {
    ...
    bus.on('log:*', function(type, packet) {
      ...
      lines.forEach(function(line) {
        ...
        if (!raw) {
          if (timestamp) process.stdout.write(chalk['dim'](chalk.bold['grey'](moment().format(timestamp) + ' ')));
          if (type === 'out') process.stdout.write(chalk.bold['green'](name) + ' ');
          else if (type === 'err') process.stdout.write(chalk.bold['red'](name) + ' ');
    ...
    setTimeout(function() {
      bus.on('process:event', function(packet) {
        if (packet.event == 'online')
          console.log(chalk.green('[pmd] App %s restarted due to file change'), packet.process.name);
      });
    }, 1000);
...
Log.jsonStream = function(Client, app_name) {
  ...
  Client.launchBus(function(err, bus) {
    ...
    bus.on('log:*', function(type, packet) {
      if (app_name && app_name != packet.process.name) return false;
...
Log.rawStream = function(Client, id, raw, timestamp, exclusive) {
  var that = this;
  ...
  Client.launchBus(function(err, bus) {
    ...
    bus.on('log:*', function(type, packet) {
      ...
      lines.forEach(function(line) {
        ...
        if (!raw) {
          if (timestamp)
            process.stdout.write('time=' + moment().format(timestamp) + ' ');
