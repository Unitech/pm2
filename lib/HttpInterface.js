      ...
      pm2.list(function(err, list) {
        ...
        if (cst.WEB_STRIP_ENV_VARS) {
          for (var i = data.processes.length - 1; i >= 0; i--) {
            ...
            // Only remove pm2_env.env if it exists
            if (proc.pm2_env && proc.pm2_env.env) {

              // Remove any ENV keys that also exist in pm2_env
              Object.keys(proc.pm2_env.env).forEach(function(env_key) {
                if (proc.pm2_env.hasOwnProperty(env_key)) {
                  delete proc.pm2_env[env_key];
                }
              });
