...
function create () {
   ...
   */
  /**
   * Mount points mounted read-only (disk images, sealed system volumes,
   * squashfs/snap...). Their usage is fixed and never actionable, so the
   * renderer would only ever show a false "disk nearly full" alarm.
   */
  async function readOnlyMounts () {
    var out = await execFileP('mount', [])
    var ro = {}
    if (!out) return ro
    out.split('\n').forEach(function (line) {
      // linux:  /dev/x on /mnt type ext4 (ro,relatime)
      // darwin: /dev/x on /mnt (apfs, local, read-only, journaled)
      var m = /^\S+ on (.+?) (?:type \S+ )?\((.*)\)\s*$/.exec(line)
      if (!m) return
      var opts = m[2].split(',').map(function (o) { return o.trim() })
      if (opts.indexOf('ro') !== -1 || opts.indexOf('read-only') !== -1) ro[m[1]] = true
    })
    return ro
  }
  ...
  async function fsUsage () {
    ...
    if (!out) return []
    var ro = await readOnlyMounts().catch(function () { return {} })
    var lines = out.split('\n')
    var res = []
    for (var i = 1; i < lines.length; i++) {
      ...
      // Real devices only, plus the root fs which may be a virtual device
      // (overlay in containers) and is always the one that matters
      if (p[0].indexOf('/dev/') !== 0 && mount !== '/') continue
      var sizeKb = parseInt(p[1], 10) || 0
      var usedKb = parseInt(p[2], 10) || 0
      var availKb = parseInt(p[3], 10) || 0
      if (ro[mount]) continue
