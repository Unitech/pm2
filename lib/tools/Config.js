...
/**
 * Filters for substitutions.
 * @type {{esc: Function, upper: Function, lower: Function, def: Function, replace: Function, substr: Function, capitalize: Function}}
 */
var filters = {
  esc       : function(){
...
/**
 * __proto__, in order to make the function / object in a chained mode.
 * @param {Boolean} store
 * @returns {{}}
 */
 ...
 * @returns {*}
 */
Config.subs = function(config, subs){
  for(var key in config){
    var val = config[key];
    if(typeof val == 'string'){
      config[key] = Config._subs(val, subs);
    }else if(Array.isArray(val)){
      config[key] = val.map(function(v){
        return Config._subs(v, subs);
      })
    }
  }
  return config;
 ...
 * @private
 */
Config._subs = function(text, data){
  ...
  return text.replace(re_literal, function(match, content, position){
    ...
    var methods = cts.filter(function(c){
      return re_filters.test(c);
    });
