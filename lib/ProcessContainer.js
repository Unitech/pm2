  ...
  Utility.startLogging(stds, function (err) {
    ...
    function getUncaughtExceptionListener(listener) {
      return function uncaughtListener(err) {
        ...
        if (stack && listener === 'unhandledRejection') {
          stack = 'You have triggered an unhandledRejection, you may have forgotten to catch a Promise rejection.\n' + stack;
        }

        logError(['std', 'err'], stack);

        // Notify master that an uncaughtException/unhandledRejection has been catched
        try {
          var errObj = {};
