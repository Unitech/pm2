...
function exec(script, stds) {
    ...
    if (msg.type === 'log:reload') {
      for (var k in stds){
        if (typeof stds[k] == 'object' && !isNaN(stds[k].fd)){
          if (stds[k].end) stds[k].end();
          else if (stds[k].destroy) stds[k].destroy();
  ...
  // Prefix every line of a (possibly multi-line) write with the date
  function prefixLogWithDate(string) {
    var ts = dayjs().format(pm2_env.log_date_format);
    var text = string.toString();
    var trailing = text.endsWith('\n');
    var lines = trailing ? text.slice(0, -1).split('\n') : text.split('\n');
  ...
  Utility.startLogging(stds, function (err) {
    ...
    // fd 1/2 are pipes to the daemon (cluster silent mode) and libuv made
    // them O_NONBLOCK when process.stdout/stderr got instantiated. Writes
    // that bypass the hooks below (fs.writeSync(1), pino/sonic-boom, native
    // addons, V8/inspector messages) would then be partial past the pipe
    // buffer (64 KB) and lose data silently, and the flag is shared with
    // children spawned with stdio 'inherit'. Make them blocking again, as
    // Node does on Windows: the hooks never write to the pipe, so console.log
    // keeps going straight to the log files and the pipe only carries raw
    // writes, which behave like the file fd they used to be
    ...
    process.stderr.write = (function(write) {
      return function(string, encoding, cb) {
        ...
        if (pm2_env.log_type && pm2_env.log_type === 'json') {
          ...
          log_data = prefixLogWithDate(string);
        else
