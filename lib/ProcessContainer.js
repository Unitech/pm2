...
function exec(script, stds) {
  ...
  if(p.basename(script).toLowerCase() == 'npm.cmd') {
    process.argv.splice(1, 1, script = p.resolve(p.dirname(script), 'node_modules/npm/bin/npm-cli.js'));
  }
  if(p.basename(script).toLowerCase() == 'npx.cmd') {
    process.argv.splice(1, 1, script = p.resolve(p.dirname(script), 'node_modules/npm/bin/npx-cli.js'));
  }
