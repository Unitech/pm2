...
/**
 * pm2_env keys owned by PM2 (ecosystem schema + daemon internal state).
 * They must never be overridden by a process environment variable of the
 * same name (e.g. a shell `name=`, `namespace=` or `cron_restart=` var).
 * `io` is excluded: it is a passthrough deliberately copied into env by API.js
 * so the child receives it as a JSON string.
 */
var PM2_CONFIG_KEYS = new Set(Object.keys(schema)
...
function isPm2ConfigKey(key) {
  return PM2_CONFIG_KEYS.has(key) || /^(pm_|axm_)/.test(key);
}

var Utility = module.exports = {
  isPm2ConfigKey : isPm2ConfigKey,
  /**
   * Flatten pm2_env.env into pm2_env (so it is exported to the child process)
   * without letting environment variables clobber PM2 configuration
   */
  extendEnv : function(destination, env) {
    if (!env || typeof env != 'object') return destination;

    Object.keys(env).forEach(function(key) {
      if (isPm2ConfigKey(key)) return;
      if (env[key] != '[object Object]')
        destination[key] = env[key];
    });
  ...
  getCanonicModuleName: function(module_name) {
    ...
    if(canonic_module_name.indexOf('http://') !== -1 || canonic_module_name.indexOf('https://') !== -1) {
      var uri = new URL(canonic_module_name);
