...
var Utility = module.exports = {
  isPm2ConfigKey : pm2Keys.isPm2ConfigKey,
  isPm2InternalKey : pm2Keys.isPm2InternalKey,
  /**
   * Flatten pm2_env.env into pm2_env (so it is exported to the child process).
   *
   * Shell variables named like a PM2 option are already stripped upstream, by
   * Common.prepareAppConf and by the --update-env merge in API.js: whatever is
   * left in `env` at this point the user asked for, so `env: { port: 3000 }`
   * has to land in the child. Only the daemon's own bookkeeping is off limits.
   */
  extendEnv : function(destination, env) {
    if (!env || typeof env != 'object') return destination;

    Object.keys(env).forEach(function(key) {
      if (pm2Keys.isPm2InternalKey(key)) return;
